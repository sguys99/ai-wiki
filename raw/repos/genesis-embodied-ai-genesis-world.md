...
> 수집 메모 — 사용자의 명시적 URL 지시에 따라 `raw.githubusercontent.com/Genesis-Embodied-AI/genesis-world/main/README.md` 를 그대로 받아 저장했다 (CLAUDE.md rule #1 의 자료 수집 예외). 본문은 원문 그대로이며 요약, 번역, 윤문하지 않았다. 수집 시각 2026-09-09. 저장소 생성 2023-10-31, 최종 push 2026-09-06, star 29,909, fork 2,854, open issue 134, default branch `main`, PyPI 패키지 `genesis-world`, 문서 `https://genesis-world.readthedocs.io`. 저장소 이름이 `Genesis` 에서 `genesis-world` 로 바뀌었고 옛 경로는 새 경로로 리다이렉트된다.
