...
> 원본 본문은 WebFetch로 가져온 충실 재현본이다 (rule #1 예외 — 사용자 명시 ingest 지시). 도식 이미지(LobeHub 아이콘 등)는 저장하지 않았다.

## 1. Claude Code Beyond the Basics

Stop treating Claude Code like a prompt-and-wait chatbot. Treat it like an autonomous agent that needs guardrails, and your whole workflow shifts. The core principle from Boris Cherny and the Anthropic team is simple: **give Claude a way to verify its own work**. Without that loop, you're the only feedback signal. With it, Claude iterates until the code actually runs. Boris pegs this single move at a 2-3x quality bump.
...
> **Pro tip:** Press `Ctrl+G` to open Claude's plan in your editor and tweak it before Claude proceeds. The plan is just text, so shape it before it becomes code.

> **Pro tip:** When Claude makes a mistake, end your prompt with "Update CLAUDE.md so you don't repeat this." Boris calls Claude "eerily good at writing rules for itself" from its own failures. Do note, this one habit compounds harder than anything else in this guide.
...
**Skills beat commands.** Both `.claude/commands/*.md` and `.claude/skills/<name>/SKILL.md` register slash commands, but skills carry supporting files, `disable-model-invocation`, allowed tools, and agent overrides. Skills are the preferred pattern for reusable work.

> **Pro tip:** Run `claude project purge ~/path/to/repo --dry-run` to see exactly what local state Claude holds for a project, handy before handing off a laptop.
...
Boris cares about two things here, and the rest is noise once you've got these down. He spent a month writing increasingly elaborate context files before returning to his fundamental framing, and the elaborate versions were worse in every measurable way.
...
> **Pro tip:** Words like `IMPORTANT` or `YOU MUST` improve adherence. Use them sparingly so they carry weight.
...
Steal from people who've already done the work. These four are the ones commonly referenced:
...
`CLAUDE.local.md` sits next to `CLAUDE.md`, loads the same way, and never leaves your machine. It goes straight into `.gitignore`.

Here's how it works. After every PR, reviewers drop comments. Instead of trying to hold them in your head, paste them into `CLAUDE.local.md` the second you read them. Over a few weeks it turns into a personal rule file tuned to the exact feedback you keep getting.
...
Loaded every session. Claude now includes the auth-failure test and updates the OpenAPI spec without you asking. Nitpick comments on your PRs dropped within two weeks.

> **Pro tip:** Keep two sections clearly separated: project-specific feedback and personal habits to correct. Mixing them makes the file harder to prune later.

> **Pro tip:** Prune after a few weeks. Anything that's become muscle memory can go. The file should capture what's still in flux; the stuff that runs on autopilot can go.
...
Drop a folder into `.claude/skills/` for project scope, or `~/.claude/skills/` if you want it globally available. The folder name itself becomes the slash command you type at the prompt. Project-scoped ones sit under `.claude/skills/<name>/`, global ones under `~/.claude/skills/<name>/`.

### 5.1 What Skills Actually Are

A skill is a reusable prompt bundle living in a folder. Inside lives a `SKILL.md` carrying frontmatter and instructions, and the folder name itself becomes the slash command you type at the prompt.
...
> **Pro tip:** Use `disable-model-invocation: true` for skills with side effects. You want `/ship` to deploy only when explicitly typed, not when Claude decides it's relevant.
...
**mattpocock/skills** (https://github.com/mattpocock/skills) is the most popular skills repo. A few to keep loaded:
...
> **Pro tip:** If you do something more than once a day, turn it into a skill. Anything you repeat is a skill waiting to be written.

> **Pro tip:** Check skills into git. They become institutional knowledge, and new engineers clone the repo and get the team's accumulated practices for free.

## 6. Building Custom Subagents

Spin one up and it'll chew through fifty files without bloating your main session, then hand back a tidy summary. Isolated context. Scoped tool permissions. Separate blast radius. Starting to use them after watching a single debugging session torch a context budget chasing imports across a monorepo means you default to them now.
...
Last Friday an almost-shipped PR had a missing null check, and that's when this agent got built.
...
Trigger it by typing `Have the pr-review agent look at my current branch.` into the session. The subagent does its work in an isolated context, and your main session doesn't get cluttered with review chatter.

A few choices worth calling out. The `tools` list is deliberately read-only, since a reviewer that can patch code starts rationalizing its own fixes instead of flagging them. The `model: opus` was picked because catching a security bug before a human reviewer does is worth the cost. By the way, the "Do NOT flag" section is what actually makes the output usable. Without it you'd drown in nitpicks about variable naming.

### 6.2 Popular Subagents Worth Stealing

Straight from the Claude Code team's own workflow, agents like `build-validator`, `code-architect`, `code-simplifier`, `oncall-guide`, and `verify-app` run daily.
...
> **Pro tip:** Chain agents: Session A implements, then call `Use the code-reviewer subagent to check the work.` The reviewer evaluates in a fresh context with no implementation bias.

> **Pro tip:** Add `isolation: worktree` to frontmatter to run the subagent in its own git worktree, especially powerful when fanning out a migration across dozens of parallel agents.
...
> **Pro tip:** A team-shared `.mcp.json` together with a few well-chosen plugins gets a new engineer productive within minutes of cloning the repo. Treat plugin choices as part of your onboarding story.
...
Let's dig deeper into the two that get reached for daily.

**`/compact` vs `/clear`:** if you're starting a genuinely new task, hit `/clear` and write a fresh brief by hand. If the next task still leans on what you just did, run `/compact` with a hint about what should survive. `/compact` is a lossy LLM summary of the session. `/clear` is your own brief, written deliberately. The distinction matters: one buries bad attempts inside context where Claude trips over them again, while the other leaves them behind entirely.
...
> **Pro tip:** Use `!` as a shell escape. `!git status` or `!npm test` runs immediately with output landing in context.

> **Pro tip:** Set `CLAUDE_CODE_AUTO_COMPACT_WINDOW=400000`. Context rot kicks in around 300-400k tokens on the 1M model, so force earlier compaction to stay sharp.

**Fan-out pattern:** write the task list first, then loop over it. Migrating about two thousand component files worked this way last quarter. Generate the list, sanity-check three of them by hand, tighten the prompt until those three come back clean, then unleash it on the rest while you go get coffee.
...
Last Tuesday one line got typed, the laptop got closed, dinner happened, then came back to a green PR. `/goal` sets a completion condition, and Claude keeps grinding until that condition holds true. Every attempt to stop triggers a check against the transcript.
...
> **Pro tip:** Combine `/goal` + auto mode + `/focus`. Write a crisp brief, set the goal, walk away. Come back to a finished PR. This is the workflow Boris and Cat Wu push for Opus 4.7.
...
**Hot storage: daily session log.** Every session that runs writes a timestamped entry into `10-Daily/<today>.md`. Wire a `Stop` hook so the append happens when the session ends. No copy paste.

**Warm storage: project notes.** Each project lives under `20-Projects/`. When you start a session, Claude reads the project README and the last two or three session logs before touching anything. Two weeks of context rehydrated in about 30 seconds.
...
> **Pro tip:** Resist installing every MCP. Each one expands the tool list Claude reasons over, and bloated tool lists hurt decision quality. Starter set: GitHub, Context7, and one or two domain-specific.

> **Pro tip:** Run `/mcp` inside Claude Code to list every active server and its connection status. First place to check when something isn't working.
...
**Parallel sessions.** Boris and the team call this the single biggest productivity unlock, and that's only agreed with after kicking against it for a week. Three to five git worktrees, each running its own Claude session. Use the agent view (`claude agents`) as a control plane so you can see who's doing what without alt-tabbing through six terminals.
...
> **Pro tip:** Never let Claude claim success without evidence, whether that's tests, screenshots, or real command output. The trust-then-verify gap is the single biggest source of bad output.
...
* How Boris Uses Claude Code — https://howborisusesclaudecode.com/ (89+ seconds straight from the creator, pulled from his X threads)
...
Claude Code clicked after quitting the practice of treating it like `ChatGPT` in a terminal. The mental model flipped from "I need to write this code" to "I need to set Claude up to write this code well." Setup is the work. Execution is verification.

A few things that have genuinely changed how code gets shipped:
...
These are personal takeaways from how Claude Code gets used day to day, so your setup and mileage will look different. Contact: arpanpatel.contact@gmail.com
