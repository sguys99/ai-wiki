...
- [x] 6-1 커리큘럼형 5편 (study_path 신설): prompt-to-loop-engineering-evolution-overview, agent-harness-engineering-overview, agent-skills-overview, lightrag-family-graph-rag-overview, gbrain-ecosystem-overview
  - 완료 (2026-09-14). 5편을 subagent 병렬로 전면 재작성했다. 합계 산문이 44,559자에서 **84,140자**로 1.89배가 됐고 표는 21개에서 **55개**(편당 4.2개에서 11.0개), wikilink는 137개에서 501개다. 편별 산문은 prompt-to-loop 7,222에서 19,591, lightrag 17,071에서 19,548, gbrain 11,143에서 16,759, harness 5,890에서 15,057, skills 2,531에서 13,185이고 5편 전부 착수값을 넘겼다. lint_style error는 315건에서 **0건**, lint_terms 16건에서 **0건**이고 lint_links와 audit_captions도 0건이다.
  - study_path 5개를 신설했다. 단계 수는 prompt-to-loop 12, harness 8, skills 8, lightrag 8, gbrain 6으로 합계 42단계이고, 사이트 빌드 `[study]`는 선언 6편 53단계 미해석 0건이다(physical-ai-overview 11단계 포함). 5편 모두 frontmatter 순서와 본문 `## 학습 경로` 번호 목록의 순서와 대상이 일치함을 담당 subagent가 스크립트로 대조했다. prereq 구조는 선형 사슬이 아니라 분기형이 기본이고, prompt-to-loop은 anthropic-2025가 4개 단계의 공통 prereq, lee-hoyeon이 3개 단계의 공통 prereq인 4단계 분기다.
  - **raw와 커버 페이지 재독으로 잡은 무근거 주장이 68건**이다(gbrain 21, lightrag 20, prompt-to-loop 11, harness 11, skills 5). 편당 13.6건으로 Phase 5의 12.4건과 같은 수준이다. 유형별로는 cross-source 무출처 유입 18건, 낡은 수치 12건, hedge 소실 12건, 일반화 7건, 권위 주장 6건, 저자 한계로 위장된 우리 평가 4건이고 나머지는 저장소 현황 stale과 캡션 라벨 오기다. **게재 학회 주장은 0건**인데 lightrag 담당이 LightRAG의 EMNLP 2025 Findings와 LeanRAG의 AAAI-26을 각 wiki frontmatter `venue` 키에서 확인했다. Phase 5가 6건을 삭제한 것과 대비되는 결과다.
  - **Phase 6이 규명한 구조적 사실 네 가지.** (1) **Lin et al.의 harness benefit은 비단조인데 저장소가 이를 정반대로 요약해 왔다.** 두 subagent가 독립적으로 검출했다. 저장소 여러 곳이 "harness engineering은 frontier 모델에서 가장 크게 회수된다"로 적었으나 측정된 정점은 중간 등급이다(SWE에서 base 20.7%인 Qwen3-235B가 19.3%p로 정점, base 74.2%인 Opus 4.6은 2.6%p). frontier는 ceiling effect로 개선 여지가 좁고 weak-tier는 별도 병목 때문에 이득이 작다. "약한 모델에서는 아예 작동하지 않는다"도 과장이다(Qwen3-32B가 SWE 4.4%p). 같은 주장이 `index.md` 301행과 306행에도 실려 있어 이번에 함께 교체했다. 곁들여 "within-agent spread 최대 5.1%p"는 SkillsBench 9.3%p를 빠뜨린 값이고, "between-agent gap 36%p"는 MCP만의 값(SWE는 53.5%p)이라 둘을 짝지어 쓸 수 없다는 것도 확인됐다. (2) **overview의 논지 자체가 거짓 전제 위에 선 사례가 나왔다.** gbrain overview는 "핵심 수치가 자료 3개 이상에서 교차 확인되어 신뢰도가 높다"를 논지로 삼았는데, 실제로는 Vectorize는 README에서, Gajjar와 Liu는 Tan 본인 발표에서, 영상은 저장소 문서에서 값을 가져와 **독립 측정이 0건**이다. 합의표를 폐기하고 출처와 자기 보고 여부를 열로 세운 표로 대체했다. (3) **cross-source 무출처 유입이 여전히 최다 유형(18건)이고 같은 프로젝트를 여러 겹으로 갖는 클러스터에 집중된다.** lightrag의 RAG-Anything 3겹 구간 5건은 전부 `hkuds-rag-anything`이 스텁 전환으로 잃은 근거(`reproduce/index.py` 등 파일 경로, `register_parser()` 등 API명, vLLM과 Ollama 예제)를 overview가 그대로 안고 있던 것이다. gbrain 8건도 전부 저장소 내부 구조 서술이다. A6, A7, B4에 이어 다섯 번째 확인이다. (4) **커버 자료 수치가 원자료와 8배 어긋난 사례.** gbrain overview가 "17,888 pages, 4,383명"으로 적었으나 raw README는 146,646 pages와 24,585명이다. 시점별 인용 표로 분리했고 어느 쪽이 현재 값인지는 저장소 자료로 판정할 수 없다.
  - 커버 확장 2건. skills가 3~4편에서 **11편**으로 늘었다(mattpocock-skills, imbad0202-academic-research-skills, zhao-2026-generative-skill-composition, yang-2026-skillopt, osmani-2026-agent-skills, hada-2026-agent-skills, google-labs-code-design-md, anthropic-2025-effective-context-engineering 편입. `microsoft-skillopt`는 `yang-2026`과 동일 프로젝트라 제외). study_path 신설 기준인 커버 5편을 채우기 위한 확장이다. 편입 중 `mattpocock-skills`는 Claude Code와 Codex의 플러그인 매니페스트 차이로 같은 저장소를 양쪽에 배포하지 못한 1차 기록이라, "cross-vendor 표준"이라는 이 페이지의 주장에 실제 경계를 댄다. 나머지 4편은 `sources:` 키를 유지하거나(3편) 신설했다(skills).
  - frontmatter 정비. `sources:` 키가 없던 skills에 11개 항목을 신설했다. 제목에서 콜론 부제를 뗀 것이 3편이다(harness "Agent Harness Engineering 개괄과 학습 경로", skills "Agent Skills 규격과 생태계 지도", gbrain "GBrain 생태계 자료 지도와 학습 경로"). tags는 근거 없는 항목을 뺐다(gbrain `memex`, lightrag `graphrag` 중복 표기).
  - index.md Overviews 절 5항목을 교체했다. 자수는 269에서 178, 228에서 177, 142에서 141, 359에서 161, 417에서 198이고 `lint_index` entry-length가 8건에서 **4건**이 됐다(잔여 4건은 6-2와 6-3 담당 페이지). 301행과 306행의 Lin 관련 잘못된 주장, 298행의 "40여 클라이언트"와 300행의 "튜토리얼" 커버 표기도 함께 정정됐다.
  - 회귀 게이트. `--category physical-ai` 두 lint와 `physical-ai-overview` 단독 두 lint 전부 exit 0이다. 사이트 `build:strict`도 exit 0이고 절별 카드 수 변화는 없다.
  - 용어집 등재 후보 16종 누적 (Phase 7-1 소관). 표기가 실제로 갈린 것은 `comprehension debt`(osmani 원어 대 kang "이해 부채"), `compounding`(원어 대 "누적되는 메모리", "복리 누적"), `skillpack`(garrytan 붙여쓰기 대 techwealth 띄어쓰기), `pairwise`(원어 21회 대 음차 7회)다. 나머지는 evolver, activation, adherence, capability-reliability gap, verification distance, base capability, community summary, chunk, dual-graph, discovery, skill library, context pointer, source of truth와 derived index, opinionated software다.
  - **사용자 확인 필요 사항 5건.** (1) `query` 표기를 lightrag 한 편에서 원어로 통일했다(오케스트레이터 지시). 저장소 다수 표기는 "질의"이고(wiki 808회 대 query 333회) 커버 8편 전부가 "질의"를 쓴다. Phase 7-1이 "질의"를 canonical로 정하면 이 페이지 한 편만 되돌리면 된다. (2) gbrain 커버를 6편으로 유지했다. GBrain을 다루는 페이지가 저장소에 10편 있고 `garrytan-gbrain-tutorials`, `tilnote-2026`, `xguru-2026`, `kmyu-2026-akb-llmwiki-gbrain-comparison` 4편이 범위 밖이다. 한계 절에 명시했다. (3) lightrag 커버를 8편으로 유지했다. `database/microsoft-graphrag`와 `database/gutierrez-2025-from-rag-to-memory-non`(HippoRAG 2)이 실재하며 한계 절에서 링크로만 가리킨다. (4) gbrain에서 커버 밖 페이지로 가던 cross-link 3건을 제거했다(PageIndex, LightRAG overview, li-2026). 검증에 커버 밖 자료가 필요해 브리프 2절에 걸린다. (5) `hall-2026-atlassians-design-md-is-here`의 production 실측(컨텍스트 확보율 80% 대 30%, 토큰 375만 대 721만)을 skills가 쓰지 않고 design-md-overview에 남겼다. 6-2에서 배분을 확정한다.
  - 다른 페이지에서 발견한 오류 6건은 편집하지 않고 Phase 7 판단 후보로 남긴다. (a) `wiki/agents/lee-hoyeon-2026-harness-engineering` 613행 표가 "14%p", 618행 산문이 "13.7"로 한 페이지 안에서 값이 갈린다(원값 52.8과 66.5). (b) 같은 stem 4행 `year: 2025`인 `walkinglabs-learn-harness-engineering`이 593행 대응표에서 2026년 담론과 같은 세대로 놓여 선후가 드러나지 않는다. (c) `wiki/applications/techwealth-hub-2026-garry-tan-gbrain-explained` 361행이 database primitive 네 가지와 verification runbook을 "저장소 문서와 일치"로 단정하나 현 README에 둘 다 없다. (d) `wiki/applications/garrytan-gbrain` 245행의 `bun install -g github:garrytan/gbrain`을 `vectorize-2026` 215행은 postinstall hook 차단으로 실패한다고 경고한다(두 페이지가 서로를 참조하지 않는다). (e) `sources/edge-2024-from-local-to-global` 133행의 "LangChain, LlamaIndex, NebulaGraph, Neo4J 통합"이 대응 wiki 페이지에 없다. (f) `wiki/database/hkuds-rag-anything` 353행이 "README에 라이선스 조항 없음"을 적는데 같은 파일 frontmatter는 `license: "MIT"`를 유지한다(페이지가 불일치를 스스로 기록한 상태).
  - 배치 공통 브리프에 부록 A를 신설했다. 배치 중 전파된 정정 사실 4건(Lin 비단조, TerminalBench 원값 인용, Osmani의 "재유도 반복" 표현, Patel 동명이인)을 담아 6-2와 6-3 담당이 같은 오류를 재생산하지 않도록 했다.
- [x] 6-2 비교형 3편 (역할 대응표 강화): headroom-context-compression-overview, design-md-overview, loop-engineering-cross-domain-overview
  - 완료 (2026-09-14). 3편을 subagent 병렬로 전면 재작성했다. 합계 산문이 9,004자에서 **31,922자**로 3.55배가 됐고 표는 7개에서 **23개**, wikilink는 37개에서 214개다. 편별 산문은 headroom 2,533에서 12,744(5.0배), loop-cross 3,598에서 10,801(3.0배), design-md 2,873에서 8,377(2.9배)이다. lint_style error는 80건에서 **0건**, lint_terms 1건에서 **0건**이고 lint_links도 0건이다.
  - **study_path 판정을 자료로 다시 했고 3편 중 2편이 비교형에서 커리큘럼형으로 바뀌었다.** 계획서는 이 셋을 비교형으로 예단했으나 담당 subagent가 (a) 난이도와 선후 관계 정렬, (b) 학습 트랙 성립, (c) 커버 5편 이상 세 기준을 자료에서 판정한 결과 headroom과 design-md가 셋 다 충족했다. 근거가 우리 판단이 아니라 **자료 자신의 서술**에 있었다는 점이 중요하다. yongkyun 페이지는 `## 관련 페이지`에서 "overview가 정리한 60~95% 절감 주장이 이 글의 재생 실험에서 지출 2.8%로 나타난다"고 적어 저장소에서 독립 실측으로의 선후를 명시하고, nedai 페이지는 `## 한계`에서 "세 방식 중 무엇을 언제 선택해야 하는지 기준이 없다"고 자인하는데 그 기준이 tosea에만 있어 선후가 성립한다. design-md 쪽에서는 hall 페이지가 "규격 본체는 google-labs-code-design-md가 담당한다"고 적고 google-labs 페이지가 실측을 hall로 넘긴다. headroom 6단계와 design-md 5단계를 신설해 저장소 전체 study_path는 선언 8편 **64단계**가 됐고 사이트 빌드 `[study]` 미해석 0건이다.
  - **loop-cross는 기준 미충족으로 study_path를 신설하지 않았다.** (c)는 커버 7편으로 충족하나 (a)와 (b)가 미충족이다. Osmani(명명)와 Kang(타임라인)은 선후가 아니라 동급 진입점이고 Sean은 입문서이면서 발행이 가장 늦다(2026-06-26). 결정적인 것은 **중복**이다. 커버 7편 중 4편이 이미 `prompt-to-loop` 12단계 경로에 순서까지 확정된 채 들어 있고 osmani는 `agent-harness` 8단계에도 있어, 같은 자료로 세 번째 번호 목록을 만들면 독자에게 서로 다른 세 순서를 제시하게 된다. 신규 편입 가능 자료는 movez, sean, ai-boost 3편뿐이라 독립 트랙이 성립하지 않는다. 대신 다섯 단계 도메인 대조표와 verifier gate 대응표, 근거 등급표를 강화했다.
  - 커버 확장 2건. headroom이 5편에서 **6편**이 됐다(`agents/yongkyun-2026-cutting-llm-token-costs-with` 편입). 계획서 배치 A7이 이 클러스터를 6편으로 계량하며 yongkyun을 "독립 실측" 담당으로 지정했는데 overview `sources:`에서 빠져 있었고, 이 페이지는 자기 보고가 아닌 유일한 독립 검증 근거라 논지에 결정적이다. design-md는 `sources:` 키를 신설하며 2편에서 **5편**이 됐고(anthropic-2025-effective-context-engineering, osmani-2026-agent-skills, hada-2026-agent-skills 편입), 편입 근거는 (c)를 채우기 위해서가 아니라 본문이 각각을 실제 근거로 쓰기 때문이다. loop-cross는 7편을 유지했다.
  - **Lin 편입 판정: 편입하지 않고 라우팅.** loop-cross의 index 항목이 "Movez는 데모, Lin은 실증"이라 적었으나 `lin-2026`은 이 페이지 `sources:`에 없었다. 담당 subagent가 세 근거로 비편입을 판정했다. (1) Lin의 통제 실험은 도메인 이식과 직교해 대조표 어느 열도 채우지 못한다. (2) 자매 페이지 `agent-harness-engineering-overview`가 이미 Lin을 커버 자료로 삼는다. (3) Lin을 빼면 **커버 7편에 통제 실험이 한 건도 없다**는 사실이 드러나 이 개괄의 중심 한계를 정확히 진술할 수 있다. Lin 관련 서술을 전부 삭제하고 한계 절의 라우팅 문장으로 대체했다.
  - **raw와 커버 페이지 재독으로 잡은 무근거 주장이 25건**이다(headroom 9, design-md 9, loop-cross 7). 편당 8.3건으로 6-1의 13.6건보다 낮은데, 커버 자료가 적고 6-1에서 자매 페이지를 먼저 정리한 효과로 보인다. 유형별로는 cross-source 무출처 유입 6건, hedge 소실 5건, 낡은 수치 4건, 저자 한계로 위장된 우리 평가 4건, 일반화 4건, 저장소 내부 구조 서술 1건, 구두점 인용 오류 1건이다.
  - **6-2가 규명한 구조적 사실 네 가지.** (1) **"여러 자료가 같은 표를 인용한다"는 합의 주장이 또 거짓이었다.** headroom overview는 "네 소개글이 모두 저장소의 같은 표를 인용한다"고 적었으나 Tosea만 4행 전부를 싣고 9bow는 3행(47% 행 누락), subratpati와 nedai는 표 자체를 싣지 않는다. 6-1의 gbrain("3개 자료 교차 확인"이 실제로는 독립 측정 0건)에 이어 두 번째다. **overview가 합의를 주장하면 인용 실태를 한 건씩 확인해야 한다**는 것이 이 Phase의 반복 교훈이다. (2) **스텁 전환 대상이 아닌 stem에서는 구현 클래스명 인용이 모두 실재했다.** A7이 headroom overview의 `SmartCrusher` 인용을 재작성 대상으로 지목했으나 전수 확인 결과 허위 0건이고, `CacheAligner`, `ContentRouter`, `CodeCompressor`, `CCR`이 전부 raw README에 있다. 정정 대상은 표기 절단(`Kompress-v2`가 `Kompress-v2-base`의 절단)과 목록 불완전(이미지 압축기 누락)뿐이다. 6-1의 lightrag와 gbrain에서 같은 유형이 대량 검출된 것은 그 stem들이 커밋 `0507ad0`의 스텁 전환 대상이었기 때문이지 서술 습관 때문이 아니다. (3) **raw 스냅샷의 한계를 저장소의 결함으로 잘못 읽은 사례.** design-md overview가 "정본은 스펙조차 repo 밖 링크에 기댄다"고 적었으나 raw README는 "The full DESIGN.md spec lives at `docs/spec.md`"라고 명시한다. 스펙은 저장소 안에 있고 우리 raw 스냅샷에 README와 PHILOSOPHY만 들어 있을 뿐이다. 함께 적힌 `Stitch`는 이 저장소 raw에 0회 등장하며 hall 기사가 전하는 사실이었다. (4) **측정 범위를 넘은 서술.** design-md overview의 "정확도도 밀린다"는 벤치마크 네 열(컨텍스트 확보율, 토큰, 시간, turn)에 없는 주장이고 디자인 품질과 정확도는 직접 측정되지 않았다. "토큰은 MCP의 두 배"도 원값 721만 대 375만이라 1.9배다.
  - 오케스트레이터 직접 처리 1건. headroom 커버를 6편으로 늘리면서 `wiki/agents/yongkyun-2026-cutting-llm-token-costs-with` 486행의 "소개글 넷을 묶은 overview"가 낡은 서술이 됐다. "소개글 넷, 독립 재측정 한 편(이 글)"으로 고쳤다. yongkyun을 소개글로 세지 않는 것은 이 페이지가 독립 실측이기 때문이다. 같은 파일의 두 lint는 exit 0이다.
  - index.md Overviews 절 3항목을 교체했다. 자수는 231에서 155, 270에서 188, 323에서 199이고 `lint_index` entry-length가 4건에서 **1건**이 됐다(잔여 1건은 6-3 담당 gstack). 304행의 "Movez는 데모, Lin은 실증" 표기도 위 판정에 맞춰 정정됐다. 제목에서 콜론 부제를 뗀 것이 3편 전부다.
  - 회귀 게이트. `--category physical-ai` 두 lint와 사이트 `build:strict` 전부 exit 0이다. `--category overviews` 잔여는 error 27건 warning 4건이고 **전부 gstack(25건)과 index.md 2건**(310행 glossary-llms 항목의 중간점과 em dash)이다. 각각 6-3과 6-5 소관이다.
  - 용어집 등재 후보 14종 추가 누적 (Phase 7-1 소관, Phase 6 합계 30종). 표기가 실제로 갈린 것은 `design token`(hall은 원어 유지, google-labs는 "토큰"으로 줄이고 LLM 쪽을 "컨텍스트 토큰"으로 구분. **두 커버 페이지 규약이 반대다**), `counterfactual`(yongkyun "반사실(counterfactual)" 대 repo "counterfactual"), `cache read`와 `cache write`(API 필드명 `cache_read`와 산문 표기 혼용)다. 나머지는 operational context, live zone, prompt caching, truncation, just-in-time, on-demand, slop, verifier gate, grader, loopable, hill climbing이다.
  - **사용자 확인 필요 사항 4건.** (1) design-md가 두 커버 페이지의 반대 규약 중 hall 쪽(디자인 값은 design token, 비용 단위는 토큰)을 택하고 요약 절에 규약을 한 문장으로 선언했다. 저장소 차원 canonical은 7-1 소관이다. (2) loop-cross가 Sean 자료의 도메인 라벨을 정정했다. 브리프는 Movez를 "유일한 코딩 밖 도메인 사례"로 전제했으나 Sean의 러닝 예시는 D2C 이커머스 고객 응대이고 코딩 예시는 Claude Code 훅 하나다. "**루프 전체를 조립한** 코딩 밖 사례는 Movez 한 편"으로 주장 범위를 좁혀 두 사실을 살렸다. (3) `wiki/agents/subratpati-2026-building-cost-efficient-agents-with`의 `## 관련 페이지`에 headroom overview 링크가 아예 없다. 커버 6편 중 유일하다. (4) hall 기사가 adaptive theming을 DESIGN.md의 값어치 시나리오로 들면서 같은 기사에서 현행 표준이 theming을 지원하지 않아 dark mode variant를 따로 만들었다고 적는다. 기사가 둘을 연결하지 않아 그 사실만 적고 해석을 더하지 않았다.
  - 다른 페이지에서 발견한 오류 2건은 편집하지 않고 Phase 7 판단 후보로 남긴다. (a) `wiki/agents/google-labs-code-design-md` 한계 절이 "평균 컨텍스트 토큰은 MCP의 약 2배"로 적는데 자매 페이지 `hall-2026`은 같은 값을 "약 1.9배"로 적는다(721만 / 375만 = 1.92). (b) 같은 파일 393행의 "기존 디자인 토큰 파이프라인"이 저장소 전체에서 이 표기의 유일한 사례이고 같은 페이지가 다른 곳에서는 design token 또는 토큰을 쓴다.
- [x] 6-3 gstack-ai-software-factory-overview: A6의 신규 3편 편입 후 재판정 (커버가 4편에서 7편이 되면 study_path 후보)
  - 완료 (2026-09-14). 산문이 2,667자에서 **9,014자**로 3.4배가 됐고 표는 1개에서 **9개**, wikilink는 8개에서 63개다. lint_style error 22건 warning 3건에서 **0건**이고 lint_terms와 lint_links도 0건이다. frontmatter에 `sources:` 4항목을 신설하고 `source_collection`을 `external`에서 `synthesis`로 고쳤다.
  - **커버 확장 판정: 확장하지 않고 4편 유지.** 계획서가 예고한 "커버 7편"은 확장 후보 3편(`donchitos-claude-code-game-studios`, `madslorentzen-ai-job-search`, `applications/garrytan-gbrain`)을 넣었을 때의 수인데, 담당 subagent가 전부 읽고 주제 기여도로 판정해 비편입을 결정했다. 앞 두 편은 **gstack에 관한 사실을 한 건도 담고 있지 않다.** overview의 합성 가치는 같은 대상에 대한 자료 간 대조에서 나오는데 두 자료는 gstack의 어떤 수치도 확인하거나 반박하거나 보충할 수 없어 필수 표인 역할 대응표를 채울 수 없고, 편입하면 브리프 3절이 금지한 개별 자료 요약 나열이 된다. 라우팅도 이미 있어(두 페이지 모두 `## 관련 페이지`에서 `garrytan-gstack`을 "같은 패턴의 다른 사례"로 링크한다) 발견 가능성 문제가 없다. 주제를 "Claude Code로 조립한 1인 운영 패턴"으로 넓히면 `agent-harness-engineering-overview`(8편)와 `prompt-to-loop`(13편)가 이미 점유한 영역과 겹친다. `garrytan-gbrain`은 `gbrain-ecosystem-overview`가 이미 `sources:`와 study_path 3단계로 보유해, 같은 stem을 두 overview의 커버로 중복 선언하면 그 자료의 수치 판정 책임이 흐려지므로 라우팅 링크로만 뒀다.
  - **study_path 판정: 신설하지 않음 (3기준 0/3).** (c)는 커버 4편으로 미달이다. (a)는 **역할 분담이 뚜렷하다는 것과 난이도 선후가 있다는 것이 다르다**는 점에서 미충족이다. headroom과 design-md가 (a)를 충족한 이유는 자료 자신이 담당 범위와 **선후**를 명시해서였는데, 여기서 순서를 명시한 문장은 hada wiki의 "원 저장소를 정리한 `garrytan-gstack`을 먼저 보고 이 페이지를 나중에 보는 편이 낫다" 한 건뿐이고 9bow는 범위 분담만, gpters는 순서 언급이 없다. 간선 하나로는 트랙이 되지 않고, 네 편은 같은 도구의 입문 수준 소개라 서로를 전제하지 않는다. (b)도 순서 부여의 실익이 낮아 미충족이다. loop-cross의 중복 사유와는 다르다 (이 네 stem은 다른 overview의 학습 경로에 들어 있지 않다). 대신 **질문별 라우팅표**("알고 싶은 것 → 먼저 볼 페이지 → 보충할 페이지" 5행)를 뒀다. 자료가 순서가 아니라 범위로 나뉘어 있다는 실측에 맞는 형태다.
  - **무근거 주장 11건**이고 cross-source 무출처 유입이 5건으로 이번에도 최다다. 가장 무거운 것은 **GPTERS 단독 수치를 저장소 수치로 귀속한 사례**다. 브라우저 데몬 성능(콜드 스타트 3~5초, 이후 100~200ms), 58MB, Playwright, Greptile을 "저장소와 가이드 측이 내놓은 값"으로 적었으나 `garrytan-gstack`의 wiki, sources, raw 3단 전부에 grep 0건이다. "하루 1만~2만 줄 단독 작성"도 저장소 raw의 Key Statistics(810배, 240배, 1,237+, 772)에 없는 9bow 단독 값이다. `/ship` 6단계와 `/retro` Team-Aware 세부도 GPTERS 단독인데 무출처로 서술돼 있었다. 일반화 2건은 "세 자료가 공통으로 짚는 출발점"과 "세 자료를 겹쳐 보면"인데, 해당 진단이 실제로 서술된 raw는 GPTERS 한 편뿐이다.
  - **A6의 4행 손질은 현재도 유효함을 재확인했다.** "원문은 28가지라 소개하지만 실제로 이름이 나오는 것은 22개"를 9bow raw에서 직접 세어(5+3+4+3+7=22) 확인했다. 다만 22는 9bow 단독 기준이고 `garrytan-gstack` raw가 `/autoplan`, `/design-shotgun`, `/design-html`, `/pair-agent` 네 개를 더 적어 **네 자료를 합치면 26개**라는 사실이 빠져 있었다. 28과 26의 간극을 신설 절로 세웠다.
  - **저장소 wiki와 sources 전체가 lint_style 0건이 됐다.** `--all` 494개 파일 중 위반 파일은 `index.md` 한 개뿐이고 error 4건(296행 em dash, 308에서 310행 중간점)이다. 착수 시점 9,202건에서 시작해 Phase 2에서 6-3까지의 배치가 전부 흡수했고 잔여는 6-5 소관이다. `lint_index --category overviews --strict`도 index 305행 교체로 **exit 0**이 됐다(entry-length 1건에서 0건).
  - 용어집 등재 후보 4종 추가 (Phase 6 합계 34종). `software factory`는 원어 13회 대 음차 11회로 거의 반반이고 gstack 4편과 이 overview에 집중돼 있어 확산 전에 고정할 값이 있다. `telemetry`는 음차 51회 대 원어 27회인데 원어 중 상당수가 코드 식별자와 URL일 수 있어 등재 전 문맥 확인이 필요하다. 나머지는 `Diff-Aware`, `opinionated`다.
  - **자동 메모리 갱신 후보 1건.** glossary-physical-ai의 `policy` 행 금지 표기가 `applies_to`에 `overviews`를 포함해, overviews 카테고리에서는 "텔레메트리 정책" 같은 **일반어 용법도 lint에 걸린다**. 메모리 `glossary-banned-term-check`에 overviews 오탐 사례로 덧붙일 값이 있다 (Phase 7-4 소관).
  - **다른 페이지에서 발견한 오류 6건은 편집하지 않고 Phase 7-5 판단 후보로 남긴다. 전부 A6이 `wiki/agents/garrytan-gstack`에서 정정한 것과 같은 유형이 형제 페이지 쪽에 남아 있는 사례다.** (a) `wiki/agents/9bow-2026-gstack-claude-code-virtual-team` 30행의 "무엇을 요청하든 같은 태도로 응답한다" 서술이 그 stem raw에 없고 GPTERS raw의 것이다. (b) 같은 파일 219행 대조표가 브라우저 데몬 수치의 출처를 `garrytan-gstack`으로 잡았으나 그 페이지에 없다(실제 출처는 GPTERS). (c) `wiki/agents/hada-2026-gstack-virtual-engineering-team` 29행이 두 서술을 `garrytan-gstack`의 것으로 적으나 둘 다 GPTERS 페이지 내용이다. (d) `wiki/agents/gpters-2026-yc-ai-agent-guide-gstack` 220행이 "`garrytan-gstack` 기준으로 명령어 28개"라 적는데 그 페이지는 28을 "한국어 해설 3편이 기록한 값"으로 표기하고 실제로 28을 적은 것은 9bow 한 편이라 2단계 재귀 귀속이 된다. (e) `wiki/agents/garrytan-gstack` 124행이 실행 요건과 설치 절차를 "한국어 해설 3편이 기록한 값"으로 적으나 hada raw에는 둘 다 없어 실제로는 2편이다. (f) 같은 파일 130행의 Windows 행 "WSL 경유. Node.js가 추가로 필요하다"는 9bow raw(Node.js만)와 gpters raw(WSL만)를 합친 문장이라 어느 원문에도 이 형태로는 없다. **이 클러스터에서 GPTERS 수치가 저장소 수치로 오인되는 경로가 아직 두 군데 열려 있다((b)와 (c)).**
  - 사용자 확인 필요 사항 1건. 커버 4편 유지가 계획서 6-3의 "커버가 7편이 되면" 기대와 다르다. 계획을 그대로 밀려면 주제를 "Claude Code로 조립한 1인 운영 패턴"으로 재정의하고 제목과 index 항목을 함께 바꿔야 한다.
