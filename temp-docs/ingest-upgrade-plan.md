...
Phase 4와 같은 지표, 같은 방법론으로 재실측했다. 범위는 `wiki/physical-ai/` 76편과 대응 sources 76편이고, 본문은 frontmatter를 제외한 나머지를 원문 그대로(공백과 마크업 포함) 센다. 착수 열은 이 문서 2절의 전수 조사 결과다.

| 지표 | 착수 | Phase 4 완료 | Phase 5 완료 |
|---|---|---|---|
| lint_style error | 1,440건 | 0건 | 0건 |
| lint_style warning | 246건 | 0건 | 0건 |
| lint_terms 경고 | 32건 | 0건 | 0건 |
| 위반 파일 | 73개 / 76 | 0개 | 0개 |
| 중간점(본문) | 1,801개 | 12개 | 12개 |
| em dash(본문) | 702개 | 0개 | 0개 |
| 표 0개 페이지 | 45개 | 0개 | 0개 |
| wiki가 sources보다 얇은 페이지 | 69개 | 1개 | 1개 |
| 본문 중앙값 | 5,625자 | 14,194자 | 14,194자 |
| frontmatter 100줄 초과 | 20개 | 7개 | 7개 |

위 표의 열 지표가 전부 Phase 4 완료 시점과 같은 값을 유지했다. 5-1부터 5-5까지의 작업은 회귀를 만들지 않았다. Phase 4 완료 열의 `lint_style` warning 값은 당시 표에 없던 항목이라 Phase 4 완료 커밋(613ccae)에서 다시 측정해 채웠다.

본문 총량만 1,174,956자에서 1,176,181자로 1,225자 늘었다. 5-1의 번역어 치환이 원인이며, 원어가 번역어보다 길어서 생긴 증가다(휴머노이드 5자에서 humanoid 8자, 처리량 3자에서 throughput 10자). 29개 파일이 3자에서 237자씩 늘었고 줄어든 파일은 없다. 중앙값이 그대로인 것은 중앙에 놓인 파일(`jo-2026-act-vla-primer`, 14,194자)이 치환 대상에 들지 않았기 때문이다.

frontmatter 줄 수가 바뀐 파일은 `sa-2026`(39줄에서 43줄) 하나뿐이다. 5-4의 fig09와 fig10 재크롭에서 caption이 길어진 결과이고 100줄 기준과는 무관해 초과 페이지 수는 7개 그대로다.

##### Phase 5 고유 계량

| 지표 | 착수 | Phase 5 완료 | 담당 항목 |
|---|---|---|---|
| frontmatter 금지 기호 (wiki 76 + sources 76) | 미조사 | 0건 | 5-3 |
| sources figures caption 금지 기호 | 미조사 | 0건 / 804개 항목 | 5-2 |
| sources figures caption 영어 전용 | 미조사 | 0건 / 804개 항목 | 5-2 |
| wiki figures caption 금지 기호와 영어 전용 | 미조사 | 0건 / 308개 항목 | 5-2 |
| glossary-physical-ai 등재 용어 | 80개 (금지 103종) | 126개 (금지 162종) | 5-1 |
| glossary-llms 등재 용어 | 35개 (금지 27종) | 36개 (금지 29종) | 5-1 |
| glossary-agents 등재 용어 | 36개 (금지 22종) | 36개 (금지 22종) | 변동 없음 |

용어집 착수 값은 배치 착수 커밋(d64bc72)에서 당시 `lint_terms.py`가 출력한 파싱 결과다. caption 판정 기준은 두 가지다. 금지 기호는 caption 값 안의 중간점과 em dash를 세고, 영어 전용은 한글이 한 글자도 없으면서 라틴 문자가 세 자 이상인 caption을 센다.

##### 잔여 항목 판정

세 건 모두 Phase 4와 같은 내용이고 정상 상태다.

- **중간점 12개는 정상이다.** `τ·A_t`, `πθ(a | o, ℓ̂) · πθ(ℓ̂ | o, ℓ)` 처럼 수식의 곱셈 연산자이고 4개 파일 6줄에 몰려 있다. 여섯 줄 모두 코드 펜스 또는 인라인 코드 안이라 lint가 마스킹해 통과시킨다. 나열 기호로 쓰인 것은 하나도 없다. sources 쪽에도 같은 성격의 5개가 3개 파일에 남아 있고 역시 전부 코드 구간이다.
- **sources보다 얇은 1건은 정상이다.** 파일럿 `brohan-2022-rt-1`(wiki 9,602자 대 sources 9,778자, 차 176자)이고 Phase 4와 수치가 같다. sources가 `## 8. 그림 후보` 표를 갖고 wiki는 규약상 갖지 않아 생긴 차이이며, sources에만 있는 실질 내용은 없다.
- **frontmatter 100줄 초과 7편은 정상이다.** curated figure가 9개에서 11개인 페이지들이고 본문 대비 비중은 전부 14~23%다.

##### 측정 방법 주석

Phase 4 계량을 재현하면서 확인한 사항 네 가지를 남긴다.

1. **`lint_terms 경고`의 착수 값을 246건에서 32건으로 정정한다.** Phase 4 표의 246은 배치 4 절 머리의 기준선 문장에 적힌 `lint_style` warning 값이다. 배치 착수 커밋에서 당시 용어집으로 `lint_terms.py`를 다시 실행하면 wiki 76편은 경고 32건(12개 파일), sources 76편은 42건(15개 파일)이다. 위 표에는 `lint_style` warning 행을 따로 두어 두 지표를 분리했다.
2. **착수 열은 두 시점이 섞여 있다.** lint 관련 세 줄(1,440건, 246건, 73개)은 파일럿 3편을 마친 뒤의 배치 착수 시점 값이고, 나머지 여섯 줄은 이 문서 2절의 진단 시점(파일럿 이전) 값이다. 배치 착수 커밋으로 나머지도 재현하면 중간점 1,764개, em dash 684개, 표 0개 42개, 얇은 페이지 72개, 본문 중앙값 5,651자, frontmatter 100줄 초과 31개다. 차이는 파일럿 3편의 재작성과 그 sources 보강에서 나온다.
3. **본문 중앙값은 두 중앙값 중 큰 쪽을 쓴다.** 파일이 76개라 중앙에 놓이는 값이 둘인데 Phase 4의 14,194자는 위쪽 값이다. 아래쪽 값은 14,117자, 두 값의 평균은 14,155.5자이며 세 정의 모두 Phase 4와 Phase 5에서 같은 값이다.
4. **본문 글자 수는 마크업을 포함한 원문 그대로를 센다.** Phase 4가 기록한 `brohan-2022-rt-1`의 wiki 9,602자와 sources 9,778자로 이 정의를 확인했다.

대응 sources 76편도 lint_style error 1,568건에서 0건, 위반 파일 75개에서 0개를 유지한다. Phase 4가 적은 1,567건은 재현값과 1건 차이이고 원인은 확인하지 못했다.

`sources/9bow-2026-world-action-model-rise.md`는 wiki 페이지가 없는 77번째 physical-ai source라 위 76편 범위 밖이다. 본문에 lint_style error 5건과 warning 3건이 남아 있으며 5-7에서 정비한다.
