...
사용:
    ...
    python3 scripts/lint_terms.py --category agents     # 그 카테고리만 (전체 스캔 후 필터)
    python3 scripts/lint_terms.py --category a,b <file> [<file> ...]   # 지정 파일 목록을 필터
...
def main():
    ...
    if categories:
        ...
        if not targets:
            print(f"[lint_terms] 대상 파일이 없습니다 (category: {', '.join(sorted(categories))}).",
                  file=sys.stderr)
    all_warnings = []
    for path in targets:
        if not path.exists():
