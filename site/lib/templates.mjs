...
// ── 학습 경로 (study_path) ────────────────────────────────────────────────────
//
// overview 페이지 frontmatter 의 study_path 를 번호 단계로 렌더한다. 본문 `## 학습 경로` 절의
// 헤딩·도입 문단·`###` 트랙 헤딩은 그대로 두고 절 안 첫 번호 목록 자리에만 들어가므로
// (markdown.mjs 의 spliceStudyPath) 여기서는 목록만 낸다.
