...
- [ ] `index.md` 항목이 `- [[category/stem|표시 이름]]: 설명 (YYYY, type)` 문법인가, `- ` 접두 포함 200자 이내인가, 금지 기호가 없는가 (항목이 두 번째 wiki로 자라는 것을 막는다. 이 줄은 사이트 홈 카드의 원천이라 문법이 어긋나면 카드가 사라진다. `lint_index.py`가 구분자, 꼬리, 길이, 링크 category와 절 일치를 검사한다)
