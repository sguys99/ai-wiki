...
3. **국소성(Locality)**: 문장을 한꺼번에 전부 재작성하지 않는다. AI 티가 있는 구간만 수술적으로 고친다.
   - **`register: wiki` carve-out**: 기술 위키 산문은 국소 span 편집만으로는 문단 흐름이 뚝뚝 끊긴다. wiki register에서는 **문단 단위 재구조화를 허용**한다(문장 순서 조정·연결·분할·병합). 단 **문서 전체 재작성은 여전히 금지**하고, 문단의 사실·주장·수치·순서는 보존한다. §1.Y "문단 흐름 복원" 레시피 참조.
4. **자연성 우선, 완벽성 차순(Natural > Perfect)**: 과하게 문학적으로 고치지 않는다. 일상 한국어 필자의 중간값 리듬을 목표.
   - **흔한 어휘 우선(plain-language)**: 번역투를 걷어내려다 잘 안 쓰는 문어체·문학체 어휘("포개다·결이 다르다·복리로·파고들다·통로")로 갈아타는 것은 **과교정**이다. 치환어는 "일반 기술 문서·블로그에서 쓰는 말인가?"를 기준으로 고른다. drift→평이 대응표는 `quick-rules.md` "흔한 어휘 우선" 절 참조.
...
### 1.Y. 문단 흐름 복원 (Paragraph-flow · register: wiki 전용)

국소 span 편집이 끝난 뒤, wiki register에서만 문단을 한 덩어리로 다시 읽고 흐름을 복원한다. 목표는 "문장은 깨끗한데 문단이 어색한" 상태 해소다. **사실·주장·수치·인용·정보 순서는 보존**하고 연결·리듬만 손본다.

- **연결 복원**: span 편집으로 접속사를 지운 자리에 논리 공백이 생겼으면, 접속사를 되살리지 말고 **문장 순서·지시어·주어 반복**으로 흐름을 잇는다.
- **문장 분할·병합**: 한 문단 안에서 지나치게 긴 문장은 쪼개고, 토막난 단문 연쇄는 한둘 병합해 호흡을 고른다(E-1 리듬과 연동).
- **topic-sentence 공식 해체**: 모든 문단이 "요약 문장 → 부연" 틀로 시작하면 기계적이다. 일부 문단은 사례·수치·질문으로 연다.
- **지시어 정리**: "이·그·이러한"이 가리키는 대상이 문단 재배치로 흐려졌으면 명사구로 되돌린다.
- **금지선**: 문단 사이로 정보를 옮기지 않는다(한 문단의 사실을 다른 문단으로 이동 금지). 문단 경계와 정보 소속은 원문 유지 — 흐름만 복원.

> 이 레시피는 `korean-style-rewriter`가 register=wiki일 때 span 윤문 직후 1패스로 수행하고, `naturalness-reviewer`가 "문단 전체 흐름·연결" 항목으로 검수한다.
...
- **`register: wiki` 밴드**: 목표 **20~35% / 경고 >45% / 중단 >60%**. wiki 산문은 문단 흐름 복원(§1.Y)까지 하므로 일반보다 변경률이 높게 나오는 게 정상이다. 5~15%에 머물면 **저윤문** — 티가 그대로 남았다는 신호이니 재검토. 상한을 올려도 안전한 이유는 `content-fidelity-auditor`가 의미 불변을 독립 감사하기 때문.
