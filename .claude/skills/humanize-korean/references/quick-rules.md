...
**Do-NOT (탐지·윤문 모두 제외):** 고유명사·제품명·모델명·기관명, 수치·날짜·단위, 큰따옴표 안 직접 인용, 법률 조문, 수학·화학·통계 표기, 영어 약어(LLM·GPU·MCP·API 등 업계 표준), 용어집(`wiki/overviews/glossary-*.md`) 등재 원어·canonical 표기.

**과윤문 가드:** 변경률 30% 초과 = 경고, 50% 초과 = 강제 중단·롤백. (**`register: wiki`는 strict로 라우팅되므로 이 monolith 룰북 미해당** — wiki 밴드 20~35%/45%/60%는 strict 파이프라인이 적용. §wiki register 예외 참조.)

**흔한 어휘 우선 (plain-language 가드):** 번역투·기계적 병렬을 걷어내려다 **일상 기술 문서에서 잘 안 쓰는 문어체·문학체 어휘로 갈아타지 않는다.** 원문보다 더 "글쟁이 티" 나면 과교정이다. 관찰된 drift → 평이 치환:

| 문학체 드리프트 (지양) | 평이 (권장) |
|---|---|
| 포개진다 / 겹겹이 두른다 | 겹친다 / 쌓는다 |
| 결이 다르다 | 성격이 다르다 |
| 복리로 쌓인다 | 계속 쌓인다 / 누적된다 |
| 파고든다 | 안으로 들어간다 / 깊이 본다 |
| ~로 흘려보낸다 | ~에 넘긴다 / ~로 보낸다 |
| 손을 대는 통로 | 건드리는 경로 |
| 무게가 실린다 | 비중이 크다 / 중심이 쏠린다 |
| 격차를 벌린다 | 차이를 키운다 |

> 규칙: 치환어를 고를 때 "이 단어를 일반 기술 블로그·문서에서 쓰나?"를 자문. 아니면 더 흔한 말로. 원문에 없던 비유·수사는 추가 금지(자체검증 6항).
...
| B-2 | 영어 어휘 직역 가능한데 그대로 | S2 | 한국어로 옮기되 업계 표준은 유지 (용어집 등재어 제외) |
| B-5 | 전문 용어 한글 직역 (wiki register 한정) | S2 | 용어집 canonical로 복원, 첫 등장 풀이 유지 |
...
> **register: wiki 예외** — wiki 산문은 strict 파이프라인이 처리하며 등급 A의 변경률 밴드가 **20~35%**로 상향된다(일반 10~25% 대신). fidelity-auditor가 의미 불변을 독립 감사하므로 더 과감한 편집을 A로 인정한다. 상세 등급은 `naturalness-reviewer` 루브릭 참조.
