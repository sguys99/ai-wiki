...
- **본문이 요약본이 아닌지** — 스크립트가 성공으로 끝나도 원문의 일부만 담길 수 있다. 본문 첫 문단이 글의 도입부인지, 마지막이 결론까지 닿는지, 글자 수가 원문 읽기 시간과 어울리는지 본다. 3인칭으로 "~라고 설명한다" 식 서술이 이어지면 요약본을 의심한다. **요약본을 raw 로 두면 이후 sources 와 wiki 가 통째로 그 위에 쌓인다** — 2026-09 에 그런 article 3편을 뒤늦게 발견해 재수집하고 sources 와 wiki 를 다시 맞춰야 했다.
- **JS 로 본문을 채우는 사이트** — Discourse 처럼 post stream 을 가상화하는 사이트는 스크립트가 맨 아래로 스크롤한 뒤 곧바로 `page.content()` 를 부르는 사이에 본문이 DOM 에서 빠진다. 본문이 수백 자로 끝나면 렌더 대기를 넣은 래퍼로 다시 받는다. 셀렉터 오선택과 `<a><img></a>` 유실도 같은 자리에서 확인한다.
