...
- **희귀·문학체 어휘 드리프트**: 번역투를 걷어내려다 일상 기술 문서에서 잘 안 쓰는 문어체·문학체 어휘로 갈아탐("포개다·결이 다르다·복리로·파고들다·통로·무게가 실린다·격차를 벌린다" 류). **register=wiki에서 특히 중시** — 사용자가 "잘 안 쓰는 단어" 불편으로 직접 지목한 실패 모드다. 이런 어휘가 3개 이상이면 이 시그널 하나만으로도 플래그하고, `over_polish_findings`에 drift→평이 치환안을 함께 기록한다.
...
- **문단 전체 흐름·연결 (register=wiki 필수)**: 문장 하나하나는 깨끗한데 문단으로 읽으면 뚝뚝 끊기지 않는가. 접속사를 지운 자리에 논리 공백이 생겼는가. topic-sentence 공식이 문단마다 기계적으로 반복되는가. 윤문가가 문단 흐름 복원 패스(`rewriting-playbook.md §1.Y`)를 실제로 수행했는지 검수하고, 미흡하면 `rewrite_round_2`로 해당 문단을 되돌린다.

> **register=wiki 판정 조정**: wiki 산문은 변경률 밴드가 20~35%(경고 >45%, 중단 >60%)로 상향된다. 변경률이 5~15%로 낮으면서 S1이 남아 있으면 "저윤문"으로 보고 `rewrite_round_2`. 반대로 희귀 어휘 드리프트(축 2)가 뜨면 `rollback_and_rewrite`로 plain-language 재작업을 지시한다.
