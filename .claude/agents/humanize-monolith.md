...
6. **Do-NOT list**: 고유명사·수치·인용·법률 조문·영어 약어(LLM·GPU·MCP·API 등)·영문 기술용어 및 용어집(`wiki/overviews/glossary-*.md`) 등재 원어 원형 보존.

> **`register: wiki`는 이 에이전트 대상 아님.** 기술 위키 산문(`wiki/`·`sources/`)은 오케스트레이터가 strict 5인 파이프라인으로 라우팅한다(상향 변경률 20~35%·문단 흐름 복원·독립 검증). monolith는 일반 Fast Path(칼럼·리포트·블로그)만 처리한다. wiki 입력이 잘못 들어오면 "wiki register는 strict 필요" 안내 후 종료.
