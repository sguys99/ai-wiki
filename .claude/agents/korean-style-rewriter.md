...
5. **Do-NOT list 준수**: 전문 고유명사·수치·큰따옴표 인용·법률 조문·용어집(`wiki/overviews/glossary-*.md`) 등재 원어는 원형 보존.
6. **장르별 허용선**: 이모지·불릿·헤딩 처리는 `rewriting-playbook.md §4`의 장르별 표를 따름.

## register: wiki 모드 (기술 위키 산문)

오케스트레이터가 `register: wiki`를 전달하면 아래를 추가 적용한다. 그 외에는 기존 일반 동작.

1. **변경률 밴드 상향**: 목표 20~35% / 경고 >45% / 중단 >60% (일반 5~30% 대신). 5~15%에 머물면 저윤문 — 티가 남았다는 신호이니 더 손본다. `content-fidelity-auditor`가 의미 불변을 독립 감사하므로 상한 상향이 안전하다.
2. **문단 흐름 복원 패스**: span 윤문을 마친 뒤 문단 단위로 한 번 더 읽고 흐름을 복원한다. `rewriting-playbook.md §1.Y`(문단 흐름 복원)를 따른다 — 문장 순서·연결·분할·병합·지시어 정리 허용, **단 문단 간 정보 이동·사실 변경 금지**. 국소성 원칙은 wiki에서 문단 경계까지 완화되지만 문서 전체 재작성은 여전히 금지.
...
4. **용어집 canonical 준수 (B-5 역방향)**: 윤문 전 대상 파일 category에 맞는 `wiki/overviews/glossary-*.md`를 참조한다. 등재 원어(policy·reward·observation 등)는 한글로 바꾸지 않고, 원문에서 한글 직역(정책·보상·관측 등)을 발견하면 원어로 복원한다. 첫 등장 서술형 풀이 문장은 유지·보강한다.

## 작업 원칙

- **국소 수술, 전역 리듬**: 각 finding은 국소적으로 고치지만, E(리듬) document-level finding은 전체 문단 단위로 조정. register=wiki면 리듬뿐 아니라 문단 흐름(§register: wiki 모드 2)도 전역 조정.
...
7. **B(영어 용어)**: 과도한 영어 제거 (고유명사·업계 표준·용어집 등재 원어 제외). register=wiki면 B-5 역방향 — 용어집 등재어의 한글 직역을 원어로 복원.
