...
4. **리스크 세 가지에 한 줄 정의를 붙였다.** 검증 부채, comprehension debt, 인지적 저항 각각에 어떤 상태를 가리키는지 설명 문장을 하나씩 달았다.
...
| comprehension debt | 내가 쓰지 않은 코드가, 내 이해 속도보다 빠르게 쌓인다 |
...
- **comprehension debt**: 내가 쓰지 않은 코드가 내 이해 속도보다 빠르게 쌓이는 상태.
- **인지적 저항**: 루프가 주는 결과를 판단 없이 수용하게 되는 상태.
