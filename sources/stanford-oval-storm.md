...
`stanford-oval/storm`은 주제 하나만 던지면 인터넷 검색으로 처음부터 Wikipedia 스타일 글을 써 내는 LLM 시스템이다. "좋은 질문을 자동으로 만들어 내는 일"을 연구 자동화의 급소로 보고, 여러 관점(perspective)에서 질문을 생성해 검색과 대화를 굴린다. STORM(자동 파이프라인)과 Co-STORM(사람이 끼어드는 협업형) 두 갈래로 나뉘며, DSPy 위에 모듈식으로 짜여 litellm이 지원하는 어떤 LM·임베더든 갈아 끼운다.
...
2. **Perspective-Guided Question Asking.** 유사 주제의 기존 글을 훑어 서로 다른 관점을 발굴하고, 관점별로 질문을 나눠 던져 조사 폭과 깊이를 함께 넓힌다.
3. **Simulated Conversation.** 위키 작성자와 주제 전문가가 인터넷 출처에 근거해 나누는 대화를 시뮬레이션해 참고문헌을 모은다.
4. **Co-STORM의 협업형 담론.** LLM 전문가 여러 명에 사회자(moderator) 에이전트와 인간 사용자가 한 대화에 참여하고, 수집한 정보를 계층적 mind map으로 실시간 정리한다.
...
- 커스터마이즈 단위는 네 모듈이다 — Knowledge Curation, Outline Generation, Article Generation, Article Polishing. 인터페이스는 `knowledge_storm/interface.py`, 구현은 `knowledge_storm/storm_wiki/modules/*`.
