...
4. **사례 기반 비교 데이터** (5페이지):
   - LangChain: GPT-5.2-Codex 고정, harness만 변경해 TerminalBench 52.8%에서 66.5%로 13.7%p 상승(슬라이드 표제는 +14%p), 30위에서 Top 5
...
| 도구 | 역할 |
|---|---|
| Skills | 반복 작업의 레시피화. `/commit`과 `/review` 같은 슬래시 커맨드 |
...
| 회사 | 수치 | 세부 |
|---|---|---|
| LangChain | TerminalBench 52.8%에서 66.5%로 13.7%p 상승(슬라이드 표제는 +14%p) | 같은 GPT-5.2-Codex 모델을 고정하고 harness만 변경했다. 추가한 것은 셀프 검증 루프, 컨텍스트 자동 수집, 둠 루프 탐지다. 순위는 30위에서 Top 5로 올랐다 |
