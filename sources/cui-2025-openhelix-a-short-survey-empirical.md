...
직접 비교 대상은 dual-system 계열이다. LCB가 `<ACT>` 토큰과 3D Diffusion Actor 조합을 처음 썼고 OpenHelix는 이 구성을 그대로 이어받으면서 학습 방식만 prompt tuning으로 바꿨다. DP-VLA는 dual-process theory를 명시적으로 끌어온 첫 사례이자 유일하게 System 2를 얼려 쓴 사례다. HiRT는 InstructBLIP + RT-1을 썼고 Robodual은 OpenVLA + DiT에 depth·tactile까지 붙였다. DexVLA는 Qwen2-VL-2B로 경량화했다. Helix는 Figure의 humanoid 모델인데 논문 공개가 없어 표에 N/A가 많다.

dual-system 바깥에서는 π0와 GR00T N1이 대조군으로 반복 등장한다. 저자들의 기준으로는 dual-system이 아니지만 GR00T N1의 frozen 학습 방식이 MLLM 학습 실험의 동기가 됐다. latent 선택 논의에서는 GR00T N1의 중간 층 임베딩, RoboFlamingo·HiRT의 maxpooling, Robodual의 다중 `<ACT>` 토큰이 갈래로 정리된다. 로보틱스 밖 사례로는 Metaquery와 LEGO가 언급된다.

기반 구성 요소로는 LLaVA(visual instruction tuning), 3D Diffuser Actor(3D scene 표현 기반 diffusion policy), CALVIN(장기 과제 시뮬레이션 벤치마크)이 있다.

## 7. 용어집 (Glossary)

- **dual-system VLA** — System 2 역할의 대형 모델과 System 1 역할의 경량 policy를 다른 주기로 돌리는 VLA 구조. 이 논문은 System 1이 실시간 perception 입력을 직접 받는 것을 판정 기준으로 세운다.
...
- **projector pre-alignment** — 위아래를 잇는 MLP projector를 먼저 학습시키는 1단계. 생략하면 성공률이 0으로 떨어진다.
...
|---|---|---|---|---|
| fig01 | 3 | dual-system VLA를 가르는 7개 설계 축 | manual | ★ wiki 권장 (architecture) |
| fig02 | 4 | 평가 환경 3종 (CALVIN / -E / -D) | caption-region | (확인 필요) |
| fig03 | 6 | MLLM 학습 방식 3종 (frozen·fine-tuning·prompt tuning) | caption-region | ★ wiki 권장 (method) |
| fig04 | 8 | 비동기 간격 1~60의 성공률 | caption-region | (확인 필요) |
| fig05 | 8 | `<ACT>` latent 의미 공간 투영 — 방향어 분포가 안 움직인다 | caption-region | ★ wiki 권장 (핵심 진단) |
...
| tab06 | 7 | projector 사전 정합 유무 (없으면 전부 0) | manual | ★ wiki 권장 (result) |
