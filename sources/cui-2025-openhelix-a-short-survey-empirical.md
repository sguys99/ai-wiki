...
figures:
  ...
  - id: fig02
    ...
    caption: "평가 환경 3종. 표준 CALVIN, 지시문을 풀어 쓴 CALVIN-E, 물체가 움직이는 CALVIN-D"
    page: 4
  ...
  - id: fig04
    ...
    caption: "asynchronous inference 간격을 1에서 60까지 바꿔도 성공률이 거의 그대로다"
    page: 8
    strategy: caption-region
    curated: false
  - id: fig05
  ...
  - id: tab04
    ...
    caption: "MLLM 학습 방식과 CLIP loss 유무의 조합"
    page: 6
    strategy: table-region
    curated: false
  - id: tab05
...
짧은 서베이, 설계 요소별 ablation, 오픈소스 모델. dual-system VLA를 이 세 덩어리로 묶은 기술 보고서다. LLaVA-7B는 완전히 얼려 두고 어휘에 추가한 `<ACT>` 토큰 하나만 학습시키며 3D Diffuser Actor를 붙인다. CALVIN ABC-D 평균 완료 길이는 3.45(표준)와 2.26(지시 확장)이다.
...
- arXiv: 2505.03912v1 (2025-05-06, cs.RO)
- 프로젝트 페이지: https://openhelix-robot.github.io/
- 분량: 12페이지, figure 7종 + table 8종
...
dual-system이라는 말은 그동안 느슨하게 쓰였다. 저자들이 여기에 판정 기준을 하나 세운다. System 1이 RGB 같은 실시간 perception 입력을 직접 받아야 dual-system이다. 이 기준으로 π0와 GR00T N1이 분류에서 빠진다. 두 시스템이 병렬로 동작하되 갱신 주기가 다른 것이 구조의 본질이라면, 느린 쪽이 정보를 넘겨주는 동안 빠른 쪽은 자기 눈으로 환경을 보고 있어야 한다는 논리다.

설계 요소는 7가지로 나눴고 요소마다 CALVIN 실험을 붙였다. MLLM 선택, policy 선택, latent representation 선택, MLLM 학습 방식, policy 학습 방식, 두 시스템의 연결 방식, asynchronous inference 방식이 그 일곱이다. 기존 논문들은 자기 조합만 보고하고 지나갔다. 그 빈자리를 통제된 비교로 채웠다.
...
기존 6종의 구성은 Table 1에 정리돼 있다. System 2 쪽은 LLaVA-7B(LCB), OpenVLA-7B(DP-VLA와 Robodual), InstructBLIP-7B(HiRT), Qwen2-VL-2B(DexVLA)로 갈린다. 학습은 LoRA fine-tuning이 다수이고 DP-VLA만 frozen이다. latent representation은 `<ACT>` 토큰, 마지막 층 hidden embedding, 그 둘의 maxpooling, action latent와 언어 feature의 결합으로 제각각이다. System 1 쪽 policy head도 3D Diffusion Actor, Transformer, RT-1, DiT, ScaledDP로 흩어져 있다. pre-training된 policy를 쓰는 곳은 LCB뿐이고 나머지는 from-scratch다.

### 실증 분석 부분

평가 기준은 CALVIN ABC→D다. 여기에 두 변형을 더 만들었다. CALVIN-E는 지시문을 풀어 쓴 버전이고 CALVIN-D는 grasping 과제에서 물체를 좌측, 전방, 대각, 원형 네 패턴으로 움직이게 한 버전이다. ablation은 1,000회 중 첫 100회만 실행해 속도를 확보했고 최종 표만 전수로 채웠다.

단일 시스템을 왜 배제했는지부터 확인한다. RoboFlamingo는 정적 조건에서 100%를 낸다. 그런데 네 동적 조건에서는 모두 0%다. 직전 6프레임으로 latent를 뽑아 LSTM에 넣는 구조라 물체가 움직이면 학습 때와 테스트 때 latent 분포가 어긋난다. 3DDA는 82/84/46/67/80으로 성능을 유지한다. 저자들도 π0와 GR00T N1을 아직 시험하지 않았다며 이 결론이 완전히 엄밀하지는 않다고 적어둔다.
...
위아래를 잇는 MLP projector에 projector pre-alignment를 거치지 않고 바로 함께 학습시키면 frozen, fine-tuning, prompt tuning 세 방식 모두 성공률이 0%다. 연결 방식 실험의 결과는 이렇게 극단적이다. projector를 먼저 학습시키는 2단계 절차는 선택 사항이 아니라 전제 조건이다.

MLLM 한 번 추론당 policy가 실행하는 스텝 수를 1에서 60까지 바꿔도 성능이 거의 안 변한다. asynchronous inference 쪽은 예상 밖이었다. 3DDA의 최장 환경 스텝이 60이니 60은 MLLM이 에피소드당 한 번만 추론하는 극단적 asynchronous inference 조건이다. 그런데도 성능이 그대로다. 동적 환경에서도 마찬가지다.
...
여기에 보조 과제를 하나 결합한다. 저자들이 multimodal reasoning learning이라 부르는 과제로, `z<ACT>`를 MLP에 통과시켜 position `a^l`, rotation `a^r`, 개폐 상태 `a^g`를 직접 예측하게 한다. position과 rotation은 L1 loss로, 개폐는 binary cross-entropy로 지도한다. 추가로 준비할 데이터가 없다. 그러면서 대형 모델이 시각 입력을 실제로 쓰지 않으면 이 과제를 못 풀게 만든다.
...
학습은 2단계로 나뉜다. 1단계에서는 MLLM과 policy를 모두 얼린 채 prompt와 projection만 2,000 iteration 학습해 임베딩을 policy의 feature 공간에 미리 맞춘다. 2단계에서 policy를 풀고 prompt, projection과 함께 100,000 iteration까지 학습한다. MLLM은 두 단계 내내 얼려 있다. 3DDA는 65,000 iteration 체크포인트를 pre-training 파라미터로 쓴다. 전체 목적함수는 `L_total = L_lm + L_policy`다.
...
MLLM 추론 간격은 Asy(10)과 Asy(60)이 3.45 대 3.44, 2.26 대 2.20으로 사실상 같다. 에피소드당 MLLM을 한 번만 추론해도 성능이 유지된다. 실제 배포 관점에서는 유리한 결과다. 위층이 환경을 제대로 안 보고 있다는 앞선 진단을 뒤집어 말한 것이기도 하다.
...
저자들이 직접 적은 미완 목록이 다섯이다. 실제 기기 배포, 충분히 빠른 downstream policy 실행, 물리 로봇 구동, humanoid 배포, humanoid 간 협업. Figure의 Helix를 오픈소스로 재현한다는 목표에서 보면 갈 길이 멀다고 스스로 인정한다.

실험 범위도 시뮬레이터 안에 머문다. real-world 실험은 "나중에 보완"으로 미뤄져 있다. 단일 시스템 배제 논거도 RoboFlamingo 하나에 기대고 있어 π0와 GR00T N1을 CALVIN-D에서 시험해야 한다고 본문에 적어둔다.

asynchronous inference에서 검증한 것은 첫 번째 패러다임(학습에서 주기를 맞추고 추론에서만 어긋나게 두는 LCB식)뿐이다. HiRT의 버퍼 기반 방식과 Robodual의 실시간 action 교체는 후속 과제로 남았다.
...
직접 비교 대상은 dual-system 계열이다. LCB가 `<ACT>` 토큰과 3D Diffusion Actor 조합을 처음 썼고 OpenHelix는 이 구성을 그대로 이어받으면서 학습 방식만 prompt tuning으로 바꿨다. DP-VLA는 dual-process theory를 명시적으로 끌어온 첫 사례이자 유일하게 System 2를 얼려 쓴 사례다. HiRT는 InstructBLIP과 RT-1을 썼고 Robodual은 OpenVLA와 DiT에 depth, tactile까지 결합했다. DexVLA는 Qwen2-VL-2B로 경량화했다. Helix는 Figure의 humanoid 모델인데 논문 공개가 없어 표에 N/A가 많다.

dual-system 바깥에서는 π0와 GR00T N1이 대조군으로 반복 등장한다. 저자들의 기준으로는 dual-system이 아니지만 GR00T N1의 frozen 학습 방식이 MLLM 학습 실험의 동기가 됐다. latent 선택 논의에서는 GR00T N1의 중간 층 임베딩, RoboFlamingo와 HiRT의 maxpooling, Robodual의 다중 `<ACT>` 토큰이 세 가지로 정리된다. 로보틱스 밖 사례로는 Metaquery와 LEGO가 언급된다.

기반 구성 요소로는 LLaVA(visual instruction tuning), 3D Diffuser Actor(3D scene 표현 기반 diffusion policy), CALVIN(long-horizon 과제 시뮬레이션 벤치마크)이 있다.

## 7. 용어집 (Glossary)

- **dual-system VLA**: System 2 역할의 대형 모델과 System 1 역할의 경량 policy를 다른 주기로 구동하는 VLA 구조. 이 논문은 System 1이 실시간 perception 입력을 직접 받는 것을 판정 기준으로 세운다.
- **MLLM**: multimodal large language model. 이미지와 텍스트를 함께 받는 대형 언어 모델로, dual-system에서 System 2 자리를 맡는다.
- **prompt tuning**: 모델 파라미터를 전부 얼린 채 어휘에 추가한 토큰의 임베딩만 학습시키는 방식. 이 논문이 dual-system 연결에 쓰는 핵심 기법이다.
- **`<ACT>` 토큰**: 지시문 끝에 붙는 학습 가능한 특수 토큰. 마지막 층에서 뽑은 이 위치의 임베딩이 아래층으로 넘어가는 latent goal이 된다.
- **CLIP loss**: MLLM 출력을 downstream 입력 형식에 맞추려고 텍스트 인코더 출력과 정렬시키는 보조 손실. fine-tuning에서는 필수지만 prompt tuning에서는 오히려 없는 쪽이 낫다.
- **multimodal reasoning learning**: `z<ACT>`로 position, rotation, 개폐를 직접 예측하게 하는 보조 과제. 대형 모델이 시각 입력을 실제로 쓰게 강제하는 장치다.
- **projector pre-alignment**: 위아래를 잇는 MLP projector를 먼저 학습시키는 1단계. 생략하면 성공률이 0%로 떨어진다.
- **CALVIN-E / CALVIN-D**: 저자들이 만든 CALVIN 변형. E는 지시문을 풀어 쓴 언어 일반화 평가, D는 물체가 네 패턴으로 움직이는 동적 환경 평가다.
- **3D Diffuser Actor (3DDA)**: 3D scene 표현을 조건으로 trajectory를 denoising하는 policy. OpenHelix의 low-level이다.
- **RoboFlamingo (RF)**: 직전 6프레임 latent를 LSTM에 넣는 단일 시스템 VLA. CALVIN-D 실험의 대조군이다.

## 8. 그림 후보 (Figure Candidates)

| id | page | caption | strategy | 추천 |
|---|---|---|---|---|
| fig01 | 3 | dual-system VLA의 7가지 핵심 설계 요소 | manual | ★ wiki 권장 (architecture) |
| fig02 | 4 | 평가 환경 3종 (CALVIN / -E / -D) | caption-region | (확인 필요) |
| fig03 | 6 | MLLM 학습 방식 3종 (fine-tuning, frozen, prompt tuning) | caption-region | ★ wiki 권장 (method) |
| fig04 | 8 | asynchronous inference 간격 1~60의 성공률 | caption-region | (확인 필요) |
| fig05 | 8 | `<ACT>` latent 의미 공간 투영. 방향어 분포가 거의 변하지 않는다 | caption-region | ★ wiki 권장 (핵심 진단) |
| fig06 | 9 | OpenHelix 전체 구조 | manual | ★ wiki 권장 (architecture) |
| fig07 | 10 | 세부 구조 (high-level MLLM + low-level policy) | caption-region | ★ wiki 권장 (architecture) |
| tab01 | 2 | 기존 dual-system VLA 6종 구성 비교 | table-region | ★ wiki 권장 (survey) |
| tab02 | 5 | CALVIN-D 단일 시스템 평가 (RF 전부 0%) | table-region | (확인 필요) |
| tab03 | 5 | policy 학습 방식 비교 | table-region | (확인 필요) |
| tab04 | 6 | MLLM 학습 × CLIP loss | table-region | (확인 필요) |
| tab05 | 7 | prompt tuning 추가 실험 (CALVIN-E) | table-region | ★ wiki 권장 (result) |
| tab06 | 7 | projector pre-alignment 유무 (생략하면 전부 0%) | manual | ★ wiki 권장 (result) |
| tab07 | 9 | MLLM 활용 방식 비교 | table-region | (확인 필요) |
| tab08 | 11 | CALVIN ABC-D 최종 결과 | table-region | ★ wiki 권장 (result) |
