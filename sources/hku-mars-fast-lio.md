...
FAST-LIO 논문 계열(FAST-LIO·FAST-LIO2)의 공식 구현 저장소다. iEKF 기반 LiDAR-inertial odometry에 ikd-Tree를 얹어 direct 방식으로 raw 점을 등록한다. Livox·Velodyne 등 다양한 LiDAR와 ROS를 지원한다.
...
FAST-LIO 2.0은 LOAM류가 거치던 feature 추출을 건너뛴다. raw 점을 map에 그대로 등록하는 direct 방식이라 미묘한 환경 특징까지 활용한다. 스캔 안의 점은 시각별로 point-wise motion compensation을 걸어 정확히 보정한다.
...
- 경량 실행: UAV onboard 컴퓨터급 저사양 장비에서 실시간으로 돈다.

## 3. 방법론 및 아키텍처 (Methodology and Architecture)

저장소는 ROS 패키지 구조다. 빌드는 catkin 기반이고 PCL(≥1.8)·Eigen(≥3.3.4)·livox_ros_driver가 사전 의존성이다. Docker 실행 스크립트도 README에 들어 있다.

실행 경로는 세 갈래다. Livox Avia를 직결할 때는 `mapping_avia.launch`를 쓴다. livox_ros_driver로 실기기를 붙여 돌린다. Livox serials나 Velodyne 같은 다른 기종은 config의 yaml(`velodyne.yaml` 등)을 바꿔 실행한다. 이쪽은 point timestamp 필드 유무에 따른 주의 사항을 따로 적어 놓았다. 세 번째는 rosbag 예제로, Avia·Velodyne 공개 rosbag을 돌려 재현하는 길이다.
...
라이선스가 GPL-2.0이라 상용 제품에 넣을 때는 조건을 따져야 한다.
...
- ikd-Tree — 점 삽입·삭제를 증분으로 처리하고 필요할 때 부분 재균형하는 KD-Tree. FAST-LIO2의 map 자료구조
- direct 방식 — feature 추출 단계를 두지 않고 raw 점을 그대로 등록에 쓰는 방식
- rosbag — ROS 토픽 스트림을 기록·재생하는 파일 형식. 재현 실험의 표준 경로
