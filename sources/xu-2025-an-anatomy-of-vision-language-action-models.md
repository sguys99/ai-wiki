...
figures:
  ...
  - id: fig02
    ...
    caption: "2022년부터 2025년까지의 VLA 모델, 데이터셋, 벤치마크 연표. 윗줄은 연도별 주요 모델(약 50개에서 80개, 다시 200개로 증가), 아랫줄은 학습용 데이터셋과 평가 벤치마크를 real world와 simulation으로 나눠 배치한다"
    page: 5
  ...
  - id: fig08
    ...
    caption: "도전 과제 5(데이터 구축과 평가 표준). 왼쪽은 데이터셋의 이질성 문제와 표현 수준 및 데이터 수준의 해법, 오른쪽은 평가 기준 불일치와 과제 난이도 확장 방향"
    page: 14
  ...
  - id: tabs1
    ...
    caption: "대표 embodied 데이터셋 정리표. simulation 중심(ALFRED, LIBERO, VLA-3D), 실제 기기 manipulation(BridgeData V2, DROID, Open X-Embodiment, AgiBot World), 사람 중심 egocentric(Ego4D, HOI4D, HD-EPIC), embodied VQA 네 부류로 embodiment와 시점, episode 수, 수집 방식을 비교한다"
    page: 25
...
- **저자**: Chao Xu, Suyu Zhang, Yang Liu, Baigui Sun 외 (IROOTECH TECHNOLOGY, Sany Group 산하 Wolf 1069 b Lab), Shan Luo (King's College London), Shujun Wang (홍콩이공대), Jan Peters (TU Darmstadt), Stefanos Zafeiriou와 Jiankang Deng (Imperial College London). 앞의 세 저자가 동등 기여.
...
- **5대 도전 과제의 심층 해부**: multi-modal alignment와 물리 세계 표현, 지시문 해석과 실시간 실행, 일반화와 지속 적응, 안전과 해석 가능성, 데이터와 평가 표준. 각각을 15개 하위 과제로 쪼개고 경쟁하는 해법 계열을 비교한 뒤 "Summary & Trends → Directions" 형식으로 다음 연구 방향을 제시한다.
- **학습 경로형 구조**: 모듈(공통 어휘) → milestone(맥락) → 도전 과제(연구 프런티어)의 3단 피라미드. 입문자는 아래부터 쌓고 숙련자는 필요한 층만 골라 읽는 것을 의도했다.
- **VLA 연표(2022~2025)**: SayCan과 RT-1의 등장부터 PaLM-E, Diffusion Policy, Open X-Embodiment, OpenVLA, π0, GR-2, 3D-VLA, 그리고 2025년의 GR00T N1, π0.5, VLA-RL, GEN-0까지를 모델과 데이터셋과 벤치마크 세 줄로 배치했다.
...
VLA 시스템을 perception(입력을 grounding된 observation으로), brain(multi-modal 융합과 planning), action(모터 명령 실행)으로 나눠 본다. 저자들은 세 모듈 모두에서 방향 전환이 진행 중이라고 정리한다. perception은 일반 visual backbone에서 언어에 alignment된 Transformer(SigLIP)로 옮겨 가는 중이고 여기에 기하 정보를 위한 DINOv2가 결합된다. brain은 pre-trained VLM으로 수렴하는 쪽이다. action은 이산 tokenization에서 연속 생성 모델링(diffusion과 flow matching)으로 무게가 옮겨 가는 중이다.

perception부터 보자. vision encoder는 CNN 계열(ResNet, EfficientNet)이 실시간이나 저자원 환경에서 여전히 쓰이지만 ViT가 주류 backbone이 됐다. ViT 안에서도 네 가지로 나뉜다. 언어 감독형(CLIP, SigLIP), 자기지도형(DINOv2), 둘을 합친 hybrid, 그리고 pre-trained VLM을 통째로 encoder로 쓰는 방식이다. hybrid는 OpenVLA, UniVLA, VLA-RL이 SigLIP과 DINOv2를 결합하는 식이다. language encoder는 BERT나 T5 같은 텍스트 전용 Transformer에서 Llama나 Gemma 같은 LLM으로, 다시 시각과 함께 pre-training된 VLM 순으로 옮겨 갔다. proprioception은 관절 상태, end-effector pose, 그리퍼 상태 같은 저차원 구조화 벡터라 MLP로 충분하다. 나머지 modality와는 FiLM 등으로 융합한다.
...
의미와 물리 현실 사이의 간극은 vision-language gap에서 시작한다. OTTER와 LIV는 시각 표현을 언어에 더 민감하게 만든다. ACT-LLM과 Look Leap은 LLM을 써서 언어를 중간 표현으로 삼는 symbolic reasoning으로 간극을 좁힌다. vision-language와 action 사이의 gap은 제어를 시퀀스 생성으로 바꾸는 end-to-end fine-tuning(RT-2, OpenVLA), 언어와 action이 공유하는 중간 표현(CLIP-RT, VoxPoser), 그리고 VLM planner와 저수준 controller를 나누는 계층 구조로 접근한다. 세 번째는 촉각과 힘과 소리까지 끌어들이는 multi-modal 융합이다. modality별 전용 encoder를 만들어 언어와 대조학습으로 맞춘다(TLA, OmniVTLA). 융합은 전 파이프라인 깊은 융합(Tactile-VLA)부터 VLM 표현을 보존하는 mixture-of-experts 융합(ForceVLA)까지 폭이 넓다.

공간 표현에서는 2D에서 출발한 pre-trained VLM에 3D를 어떻게 넣느냐가 문제다. 표현 후보는 2.5D depth map, point cloud(PointVLA, GeoVLA, FP3), voxel과 occupancy grid(OccLLaMA, RoboMM), 그리고 3D point의 움직임을 예측하는 4D trajectory(ARM4R)다. 아키텍처 통합에서 PointVLA와 SpatialVLA는 backbone을 건드리지 않고 adapter만 주입한다. BridgeVLA와 OG-VLA는 3D를 2D로 되돌려 렌더링하는 우회를 택하고 VoxPoser와 Gemini Robotics는 큰 멀티모달 모델의 추론 능력에 맡긴다.
...
### 3.3 도전 과제 2: 지시문 해석과 planning과 실시간 실행

모호하거나 여러 modality가 섞인 지시문을 파싱하는 문제(OE-VLA, TinkAct), long-horizon 과제를 실행 가능한 단위로 쪼개는 계층적 planning과 skill library(Long-VLA, π0.5, Hi Robot, VLP, RoboBrain), 실행 중 오류를 잡아내고 복구하는 문제, 그리고 이 모든 것을 실시간 안에 끝내야 하는 연산 제약이다.

복구는 사람을 루프에 넣는 방식(Yell At Your Robot, CLIP-RT)과 모델이 스스로 이상 상태를 감지해 고치는 방식으로 나뉜다. CorrectNav는 자기 오류 trajectory를 모아 계속 fine-tuning하고 FPC-VLA는 VLM으로 핵심 action의 타당성을 평가하며 Agentic Robot은 plan-act-verify closed-loop을 실행한다. 실시간성에는 압축과 양자화로 대응한다. BitVLA는 1비트를 쓰고 SQAP-VLA는 지각적 pruning으로 약 2배 가속과 메모리 절반을 얻는다. 경량 backbone(NORA, TinyVLA), adapter로 지식을 작은 policy에 옮기는 방식(VLA-Adapter), attention 자체를 바꾸는 방식(RoboMamba)도 있다.

### 3.4 도전 과제 3: 일반화와 지속 적응

open-world 일반화, continual learning, sim2real, 온라인 강화학습을 다룬다.

continual learning에서는 새 과제를 배우다 기존 능력이 지워지는 catastrophic forgetting이 핵심 문제다. 해법은 새 skill에 전용 파라미터를 주는 격리와 확장(prompt나 codebook 항목 추가, InstructVLA의 mixture-of-experts 라우팅, iManip의 skill별 가중치 추가), 그리고 과거 샘플 일부를 다시 학습에 섞는 replay(ExpReS-VLA의 압축 경험 replay, iManip의 시간 기반 replay)로 나뉜다.

sim2real에서 ManiSkill3는 GPU 병렬 렌더링과 domain randomization으로 시뮬레이션 충실도를 높인다. SLIM은 RGB를 segmentation과 depth로 압축해 policy를 표현 수준에서 둔감하게 만든다. DreamGen과 RynnVLA-001은 물리 엔진 대신 실제 데이터로 학습한 world model에 맡기는 데이터 주도 시뮬레이터다.

강화학습 적용은 표본 효율과 reward 설계가 발목을 잡는다. 앞의 문제는 VLA가 이미 가진 사전 지식을 RL에 주입해 푼다. RLDG는 과제별 전문 RL policy의 고품질 trajectory를 일반 VLA로 distillation하고, iRe-VLA는 backbone을 얼린 채 가벼운 action head만 학습하는 단계와 성공 trajectory로 지도학습하는 단계를 번갈아 수행한다. reward 설계는 VLM과 LLM에 맡기는 흐름이다. VLM-RMs와 RoboCLIP은 시각 상태와 목표 서술의 유사도로 reward를 뽑는다. RL-VLM-F는 GPT-4V로 observation 쌍을 비교하고 GRAPE는 단계별 선호를 생성한다. 둘 다 VLM을 심판으로 세워 trajectory를 순위 매기는 방식이다. Eureka는 LLM에 실행 가능한 reward 함수를 짜게 한다.

### 3.5 도전 과제 4: 안전과 해석 가능성과 신뢰
...
해석 가능성은 chain-of-thought를 언어로 노출하거나(Diffusion-VLA, ECoT) 시각 subgoal image로 보여준다(CoT-VLA). 계층 구조에서는 planner가 내놓는 중간 지시문 자체를 설명으로 삼는다(RT-H, HiRobot). DIARC-OpenVLA는 학습된 블랙박스 모델의 은닉층에 linear probe를 달아 기호 상태를 읽어낸다.

### 3.6 도전 과제 5: 데이터와 평가 표준

데이터의 이질성(시뮬레이션 대 실제 기기, 서로 다른 embodiment와 제어 인터페이스)은 표현 수준과 데이터 수준과 표준화 수준으로 나눠 다룬다. 표현 수준에서는 연속 동작을 의미가 일관된 latent action 토큰으로 사상하거나(LAPA, Moto, UniVLA) 지각과 추론과 제어를 아우르는 공유 의미 공간을 만든다(RDT-1B, AgiBot World). 데이터 수준에서는 생성 모델로 시각 다양성을 늘린다(CACTI, GenAug, ROSIE). Re-Mix는 성격이 다른 데이터셋의 혼합 비율을 성능 피드백으로 조정한다. 표준화 수준에서는 수집 프로토콜과 동기화를 규격화하거나(RH20T, BridgeData V2) 데이터셋을 대규모로 합친다(Open X-Embodiment).

평가에서는 지표와 실험 설정이 제각각이다. 벤치마크는 짧은 과제에 머물러 고차 추론을 시험하지 못하고 일반화도 체계적으로 재지 못한다. 대응으로 Benchmarking VLAs는 입출력과 지표와 로봇 커버리지를 통일하려 하고 EUQ는 성공과 실패의 이진값 너머의 과정 품질을 사람이 채점한다. long-horizon 과제의 CALVIN, lifelong learning 전용 LIBERO, 1인칭과 3인칭 동기화의 Ego-Exo4D는 과제의 폭과 깊이를 넓혔다. 프런티어 일반화를 겨냥한 어려운 시험으로는 From Intention to Execution이 있다.

## 4. 주요 결과와 벤치마크 (Key Results and Benchmarks)

서베이라 자체 실험은 없다. 대신 세 개의 참조표가 결과물에 해당한다.

- **Table S1(데이터셋)**: simulation 중심(ALFRED 시연 데이터 8,055개, LIBERO 약 6,500 episode, VLA-3D), 실제 기기 manipulation(BridgeData V2 60,096 trajectory, DROID 약 7만 6천 개(약 350시간), Open X-Embodiment 100만 개 이상 trajectory와 22종 로봇, AgiBot World 100만 개 이상 trajectory), 사람 중심 egocentric(Ego4D 약 3,700시간, HOI4D, HD-EPIC), embodied VQA(MT-EQA, EgoTaskQA, EmbodiedEval)를 embodiment와 시점, 규모, 수집 방식으로 비교한다.
- **Table S2(벤치마크)**: 대표 embodied 벤치마크의 구성 비교.
- **Table S3(milestone 모델)**: 2021년 이전 EmbodiedQA와 CLIPort부터 SayCan, RT-1, RT-2(2022), PaLM-E, Diffusion Policy(2023), 3D-VLA, Octo, OpenVLA, GR-2, π0(2024), Humanoid-VLA, GR00T N1, PointVLA, CoT-VLA, π0.5, LUMOS, VLA-RL, Cosmos-R1(2025)까지를 훑는다. 모델마다 perception과 brain과 action 구성, 학습 방식(BC, RL, predictive modeling), 주 데이터셋, 평가 환경을 한 줄로 정리했다. 여기서 action 표현이 discrete(autoregressive)에서 continuous(DDPM, flow matching)로 옮겨 간 흐름이 보인다. brain도 LSTM과 Transformer에서 VLM으로 넘어가는 추세다.
...
- **데이터와 평가**: 실세계 수집은 본질적으로 확장이 어렵고 잡음이 많다. 저자들은 시뮬레이션을 1차 데이터 공장으로 삼고 실제 데이터는 시뮬레이터의 물리와 렌더링을 보정하는 alignment 용도로 쓰자고 한다. 실패 trajectory를 버리지 말고 negative mining과 대조학습의 재료로 삼자는 제안, 그리고 이진 성공률 대신 안전 여유와 효율과 교란 내성까지 재는 진단형 stress test로 평가를 바꾸자는 제안이 따른다.
...
- wiki 내부: [[physical-ai/kawaharazuka-2025-vision-language-action-models-for-robotics]]는 이 논문이 참고문헌 [8]로 인용한 full-stack 관점 서베이로, 아키텍처 분류와 하드웨어 및 데이터 수집을 더 자세히 다룬다. 도전 과제 중심의 이 논문과 상호 보완적이다.
...
| fig02 | 5 | 2022~2025 VLA 모델과 데이터셋과 벤치마크 연표 | caption-region | ★ wiki 권장 (역사) |
| fig03 | 6 | 5대 도전 과제와 15개 하위 과제 taxonomy (논문 지도) | manual | ★ wiki 권장 (핵심) |
| fig04 | 6 | 도전 과제 1의 alignment, 공간 표현, world model 3단계 | caption-region | ★ wiki 권장 (도전 과제 1) |
| fig05 | 9 | 도전 과제 2의 지시문 해석, planning, 복구, 실시간 실행 | caption-region | ★ wiki 권장 (도전 과제 2) |
| fig06 | 11 | 도전 과제 3의 open-world, continual learning, sim2real, RL | caption-region | ★ wiki 권장 (일반화) |
| fig07 | 13 | 도전 과제 4의 안전, 해석 가능성, 신뢰 (크롭에 본문 텍스트가 섞여 제외) | caption-region | (확인 필요) |
| fig08 | 14 | 도전 과제 5의 데이터 구축과 평가 표준 | caption-region | ★ wiki 권장 (도전 과제 5) |
