...
WALL 계열 embodied foundation model의 학습과 추론 코드를 담은 X Square Robot의 공개 저장소다. LeRobot 데이터 준비, flow matching과 FAST 두 가지 action 브랜치, WebSocket 서빙과 LIBERO 평가까지 배포에 필요한 것을 한 묶음으로 낸다.

## 1. 자료 정보 (Document Information)

- 저장소: https://github.com/X-Square-Robot/wall-x
- 조직: X-Square-Robot, 라이선스 Apache-2.0
- 생성 2025년 9월 4일, 최종 push 2026년 8월 18일, star 1,240개 (수집 시점)
...
WebSocket 서빙과 open-loop 평가 도구를 함께 낸다. 서버를 띄우고 LeRobot ground truth와 예측을 비교해 그림으로 뽑는 스크립트까지 있어서, 실제 기기 없이도 policy 출력의 형태를 확인할 수 있다. policy는 현재 observation을 받아 다음 action을 정하는 함수를 말한다.
...
설정 항목으로는 GPU 구성, 모델과 데이터 경로, 로봇 자유도, 학습 하이퍼파라미터가 있다고 적는다. 자유도를 설정으로 받는다는 게 cross-embodiment 학습을 전제한 구조다. embodiment는 로봇의 물리적 형상과 그에 딸린 제어 API 구성을 뜻한다.

Wall-OSS-0.5의 fine-tuning, 정규화, LIBERO 평가, open-loop WebSocket 평가는 모두 `workspace/README.md`로 미룬다. 이 README에는 절차가 없다.

### 추론과 서빙

가장 작은 예제는 `scripts/fake_inference.py`다. `Qwen2_5_VLMoEForAction.from_pretrained()`로 모델을 올리고, proprioception 입력과 attention mask와 데이터셋 스펙을 준비해 bf16 `validate` 모드로 한 번 실행한 뒤 출력 shape과 수치 안정성을 확인한다. proprioception은 관절 각도 같은 로봇 자신의 상태 감각 입력이다. 클래스 이름 `VLMoEForAction`이 mixture-of-experts 구조를 그대로 담고 있다.

시뮬레이터 평가는 `bash scripts/run_libero.sh <체크포인트>`로 실행한다. LIBERO 설치는 선택이다.

서빙은 `scripts/run_serving.sh`에 체크포인트 경로, 학습 설정 경로, 포트를 넘겨 띄운다. 기본값은 모델이 낸 action chunk를 가공 없이 돌려주는 동작이라 open-loop 평가에 맞고, 로봇이 바로 먹을 수 있는 형태가 필요하면 `--serialize-actions`를 붙인다. 래퍼에 기본 체크포인트 경로가 없다는 점도 명시돼 있다.

open-loop 비교는 `scripts/draw_openloop_plot.py`가 맡는다. 실행 중인 WebSocket 서버에 붙어 LeRobot 데이터셋의 정답과 예측을 episode 단위로 견준다.

## 4. 주요 결과와 벤치마크 (Key Results and Benchmarks)

README에 성능 수치가 없다. 벤치마크는 논문 두 편이 담당하고 이 저장소는 재현 경로만 낸다. LIBERO 실행 스크립트가 들어 있으므로 시뮬레이터 성적은 직접 실행해 확인하는 구조다.

## 5. 한계와 향후 과제 (Limitations and Future Work)

설정 문서가 `workspace/README.md`와 `scripts/README.md`로 흩어져 있어 이 README만으로는 학습을 끝까지 굴릴 수 없다. 특히 로봇 자유도 설정과 정규화 절차가 그 두 문서에 있다.
...
- 한국어 해설 ([[physical-ai/jo-2026-wall-oss-vla-primer]])과 프로젝트 페이지 ([[physical-ai/x2robot-2025-wall-oss-project-page]]): 같은 모델을 다루는 다른 자료
...
| `Qwen2_5_VLMoEForAction` | 체크포인트를 올리는 모델 클래스. Qwen2.5-VL backbone에 mixture-of-experts와 action head를 결합했음을 이름에 담았다 |
