...
GeekNews(news.hada.io)에 xguru가 올린 GBrain 한국어 소개글이다. GBrain을 "분산된 마크다운 파일들을 Postgres 기반 지식 베이스로 통합"하고 벡터와 키워드 검색을 결합한 hybrid search를 제공하는 도구로 정의한 뒤, 핵심 기능 5개와 기술 스택, v0.2.0 변경점 3개를 나열한다.
...
1. GBrain을 한 문장으로 압축한다. "분산된 마크다운 파일들을 Postgres 기반 지식 베이스로 통합"하고 벡터와 키워드 검색을 결합한 hybrid search를 제공한다는 정의다.
2. 핵심 기능 5개를 목록으로 나열한다. 다중 질의 확장, AI 에이전트 스킬 7개, MCP 서버 도구 20개, 3단계 chunking 전략, 다른 도구에서의 마이그레이션이다.
...
- 기능마다 수치만 있고 내부 동작이 없다. 3단계 chunking의 각 단계, MCP 도구 20개의 목록, 마이그레이션 절차가 모두 빠져 있다.
...
이 wiki가 보유한 GBrain 페이지 가운데 chunking 전략, 마이그레이션 대상 도구, 다중 질의 확장에 쓰는 모델을 기록한 것은 이 글뿐이다. 저장소 페이지와 tilnote 요약에는 해당 서술이 없다.

## 7. 용어집 (Glossary)

- **hybrid search**: 벡터 유사도 검색과 키워드 검색을 함께 수행하고 두 결과를 하나의 순위로 합치는 방식이다.
- **다중 질의 확장**: 질의(query) 하나를 여러 개의 변형 질의로 늘려 각각 검색한 뒤 결과를 합치는 기법이다.
- **chunking**: 문서를 검색과 임베딩 단위로 잘게 나누는 전처리다.
