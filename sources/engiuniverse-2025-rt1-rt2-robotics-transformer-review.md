...
- 자막: 자동 생성(ko-orig). 수동 자막이 아니어서 발음이 비슷한 전문용어가 종종 다르게 옮겨진다 — EfficientNet→이피시언트넷, PaLI-X→팔리엑스, PaLM-E→파미, FiLM→필름, RT-1→알티어. 숫자와 고유명사는 아래 6절의 원논문 wiki 페이지와 대조해 확인했다
- 형식: RT-1과 RT-2 두 논문을 순서대로 깊이 들어가는 리뷰. 여러 편을 훑는 서베이가 아니라 두 논문의 아키텍처 대조에 집중한다. 같은 채널이 8개월 뒤 올린 `videos/engiuniverse-2025-14-key-physical-ai-papers`는 이 영상이 다룬 RT-1·RT-2를 출발점 삼아 12편으로 확장한 서베이다

## 2. 주요 기여 (Key Contributions)

영상은 RT-1·RT-2를 각각 새 논문으로 소개하는 대신 "왜 로봇 제어에 파운데이션 모델이 필요한가"라는 질문에서 출발해 둘을 하나의 진화선으로 묶는다. 도입부는 특정 태스크에 맞춰 훈련된 기존 로봇이 부품 형태나 색상이 조금만 바뀌어도 재학습이 필요한 이유를 짚고 GPT·CLIP이 NLP·CV에서 이미 증명한 일반화 능력을 로봇에 옮기면 데이터 수집 비용 절감, 범용성 확보, 제로샷·퓨샷 학습이라는 세 이점을 얻는다는 틀을 세운다. 이 틀 위에 RT-1은 "언어로 조건화된 CNN", RT-2는 "행동을 언어로 다루는 VLM"으로 대응시켜 두 모델의 구조 차이가 어디서 비롯됐는지 설명한다.
...
RT-1은 이미지와 자연어 지시 두 가지를 입력받는다. 로봇 카메라가 찍은 300×300×3 이미지는 사전학습된 EfficientNet-B3를 지나며 9×9 그리드, 각 위치 512차원의 피처맵으로 압축된다. 동시에 자연어 지시는 Universal Sentence Encoder를 거쳐 512차원 벡터가 된다. 이 둘을 잇는 게 FiLM 레이어다. 언어 임베딩에서 뽑아낸 감마(γ)·베타(β) 값을 이미지 피처맵의 각 채널에 곱하고 더해서 지시와 관련된 채널(예: "사과를 집어라"라면 사과 모양에 반응하는 채널)은 강조하고 나머지는 억제한다.
...
RT-2도 이미지와 언어 지시를 받아 행동을 출력한다는 입출력 구조는 같지만 내부는 완전히 다르다. RT-1처럼 이미지와 언어를 따로 처리한 뒤 FiLM으로 붙이는 대신, 사전학습된 VLM(PaLI-X 또는 PaLM-E) 하나가 전체 처리를 통합한다. 명령 문장은 토크나이저를 거쳐 정수 토큰이 되고 임베딩 레이어를 지나 벡터로 바뀐다. 이미지는 Vision Transformer 인코더로 들어가 224×224 이미지가 16×16 패치 196개로 잘리고 각 패치가 768차원 벡터로 평탄화된다.

RT-2의 핵심은 이렇게 만든 텍스트 토큰과 이미지 토큰을 같은 시퀀스로 이어 붙여 하나의 멀티모달 Transformer에 넣는다는 점이다. 인코더에서 크로스 어텐션으로 이미지·텍스트 토큰이 서로를 참조하고("red"라는 단어가 이미지 속 빨간 물체에 강하게 반응하는 식), 디코더는 문장을 생성하듯 숫자 토큰 시퀀스를 만든다. 이 숫자들이 곧 로봇 행동이다 — RT-1과 동일하게 11개 항목·0~255 양자화지만 VLM의 텍스트 어휘 안에 정수 토큰으로 끼워 넣어 행동을 "언어의 일부"처럼 다룬다는 점이 다르다.
...
| 항목 | 수치 |
|---|---|
| RT-1 학습 데이터 | 로봇 13대, 17개월, 약 13만 개 실로봇 시연 |
| RT-1 모델 크기 | 35M 파라미터 |
...
영상은 아키텍처 설명에 집중하고 두 논문의 정량 벤치마크(seen/unseen 성공률, distractor·background 강건성, emergent skill 비교 등)는 다루지 않는다. 수치가 필요하면 6절에 링크한 원논문 wiki 페이지의 결과 절을 봐야 한다.
...
- **FiLM (Feature-wise Linear Modulation)**: 언어 임베딩에서 뽑은 채널별 스케일(γ)·시프트(β) 값으로 CNN 피처맵을 조건화하는 레이어. 영상은 "필름 레이어"로 표기
- **EfficientNet-B3**: RT-1이 이미지 인코더로 쓰는 사전학습 CNN. 자막은 "이피시언트넷 B3"로 표기
- **TokenLearner**: 81개 비전 토큰을 8개로 압축하는 학습형 토큰 선별 모듈
- **Universal Sentence Encoder (USE)**: RT-1이 자연어 지시를 512차원 벡터로 인코딩하는 사전학습 문장 인코더
- **PaLI-X / PaLM-E**: RT-2의 backbone VLM 두 종류. 각각 55B·12B 파라미터. 자막은 "팔리엑스"·"파미"로 표기
- **Co-fine-tuning**: 웹 규모 vision-language 데이터와 로봇 시연 데이터를 같은 배치에 섞어 함께 파인튜닝하는 RT-2의 학습 전략. 자막은 "코파인튜닝"으로 표기
- **Chain-of-thought 기반 제어**: 행동 토큰을 내기 전에 추론 과정을 텍스트로 먼저 생성하게 하는 RT-2의 확장 방식
