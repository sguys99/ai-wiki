...
RT-2는 이 VLM을 로봇 제어에 그대로 쓰지 않고 원래의 웹 규모 vision-language 데이터와 로봇 시연 데이터를 같은 배치에 섞어 함께 fine-tuning하는 co-fine-tuning으로 학습한다. 웹 지식을 유지하면서 로봇 행동 감각을 더하는 절충이다. 그 결과 RT-2는 "사과를 3이라는 숫자 위에 올려라"(숫자 인식), "피곤해, 마실 것 좀 줘"(에너지 드링크를 음료로 추론), "가장 오른쪽 물체를 가져와"(공간 비교) 같은 명령을 학습 데이터에 없던 조합으로도 수행한다. 더 나아가 "망치가 필요해 → 주변에 망치가 없어 → 돌이 있네 → 돌을 망치로 쓰자"처럼 action token에 앞서 추론 과정을 텍스트로 먼저 생성하는 사례도 보여준다.
...
| 비전 backbone | EfficientNet-B3 + FiLM | PaLI-X 또는 PaLM-E (VLM 통합) |
...
자동 생성 자막이라 전문용어 오인식이 잦다. 7절 용어집은 원논문과 대조해 정정한 표기이고 "1년의 리즈닝"처럼 문맥상 의미는 짐작되지만 정확한 원문을 복원하지 못한 구간도 일부 남아 있다. <!-- lint-terms: ignore 자동 자막 오인식 사례를 원문 그대로 인용한 문장이다 -->
...
- **Co-fine-tuning**: 웹 규모 vision-language 데이터와 로봇 시연 데이터를 같은 배치에 섞어 함께 fine-tuning하는 RT-2의 학습 전략. 자막은 "코파인튜닝"으로 표기 <!-- lint-terms: ignore 자동 자막이 표기한 "코파인튜닝"을 원문 그대로 인용한 대응 기록이다 -->
