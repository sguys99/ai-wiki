...
Faster-LIO를 해설한 29분짜리 한국어 발표다. FAST-LIO2가 쓰던 map 자료구조 ikd-Tree를 해시 기반 희소 voxel 구조 iVox로 바꿔 검색과 갱신을 가볍게 만든 것이 Faster-LIO다. 발표는 iVox와 iVox-PHC의 자료구조, kNN 검색, LRU 삭제, 복잡도, 효율과 정확도 실험까지 논문 순서를 그대로 따라간다.
...
- kNN 검색은 query point가 속한 voxel과 인접 voxel(6개, 18개, 26개로 확장)을 해시 맵에서 곧바로 찾아 후보를 모은 다음 그중 최적의 k개 이웃을 고른다.
...
- 삽입과 kNN 시간 그래프: iVox 계열이 삽입에서 가장 빠르고 kNN은 작거나 중간 크기 문제에서 유리하다. 문제가 아주 커지면 다른 알고리즘에 밀린다.
- recall 대 처리 시간을 보면 낮은 recall 시나리오에서 iVox가 특히 빠르다. 높은 recall을 원하면 voxel을 키우거나 검색 범위를 넓혀야 해서 iVox가 좋은 선택이 아닐 수 있다.
- 각 데이터셋에서 시간을 가장 많이 쓰는 단계는 Kalman filter/ICP다. 여기서 Faster-LIO가 FAST-LIO2보다 눈에 띄게 적게 쓴다.
- APE와 RPE 비교에서는 FAST-LIO2와 비슷하거나 데이터에 따라 근소하게 밀리는 수준이다. 정확도는 비슷한데 속도는 더 빠르다는 것이 논문 주장이다.
...
- 자동 자막에서 옮긴 내용이라 수치 인용은 원 논문으로 확인해야 한다. 원 논문 PDF는 아직 wiki에 없다. 따라서 이 페이지가 Faster-LIO의 유일한 근거 자료다.
