...
figures:
  ...
  - id: fig03
    ...
    caption: "학습과 추론 파이프라인 (논문 Figure 4 캡처)"
    strategy: fetched
    curated: false
  - id: fig04
  ...
  - id: fig08
    ...
    caption: "ID와 OOD task progress 비교 막대 (논문 Figure 7 캡처)"
    strategy: fetched
    curated: false
  - id: fig09
  ...
  - id: fig10
    ...
    caption: "block-spell의 CoT와 subtask 4스텝 (논문 Figure 6 하위 행 캡처)"
    strategy: fetched
    curated: false
  - id: fig11
...
수식 해설이 이 글의 실질이다. Inspiration 손실의 두 항을 VQA 학습과 이산 action 학습으로 갈라 각각 무엇을 예측하는지 적고, Integration 손실에서 xt와 h, (ε−x0)가 각각 무엇인지 하나씩 풀어 준다. Uni-CoT 통합 손실에서는 v, x, c, a1:T, y, Fθ, Hθ, λ 여덟 기호를 표처럼 나열한다. 논문 본문에서 한 문단으로 지나가는 기호 정의를 입문자가 따라갈 수 있게 늘려 놓았다.

FAST tokenization을 DCT와 양자화, BPE 세 단계로 나눠 설명한 대목도 논문에는 없는 부연이다. BPE를 "자연어 처리에서 subword로 나누듯 자주 나타나는 동작 패턴을 하나의 동작 단어로 묶는다"로 옮긴다.
...
VLM의 기존 FFN을 재사용하면서 embodied VQA로 공간 추론을 키우는 가지와, 이산 action 모델링으로 거친 action 감각을 심는 가지가 병렬로 진행된다.
...
이산 토큰 예측을 flow matching으로 교체한다. 노이즈 스케줄에서 x0가 도달할 정답 action, ε이 Gaussian noise 출발점이고 ρ를 균등하게 뽑지 않고 Beta 분포로 0쪽에 몰아 준다.
...
논문에 없는 해석도 하나 붙는다. WALL-OSS가 추론과 실행을 병렬로 구동해 자기 action이 환경에 미친 영향을 시각 데이터로 곧바로 되받는 피드백 루프를 만든다는 서술이다. 오류가 나도 embodied VQA로 현재 상태를 다시 읽고 c를 고쳐 trajectory를 실시간 보정할 수 있다고 본다. trajectory는 observation과 action이 시간순으로 이어진 실행 기록이다. 논문 본문에는 이만큼 강한 진술이 없으므로 이 대목은 글쓴이의 확장으로 읽는 게 맞다.

### 데이터

자체 수집 action, open-source action, 멀티모달 VQA 세 가지를 논문과 같은 순서로 설명한다. 일반 VQA는 action head를 직접 최적화하기보다 VLM의 기존 인지 능력을 지키는 역할이라며 weight drift 방지와 묶어 설명한다.
...
논문의 한계 서술을 세 항목으로 옮긴다. 고수준 의미 이해가 아니라 미세한 물리 조작 제어에서 한계가 있다는 것, 정렬된 3D 데이터가 희소하고 공개 3D 기반 모델의 공간 정확도가 모자란다는 것, 그리고 계획 능력이 전체 학습 프레임의 1%짜리 CoT와 subtask 감독에 의존하고 있어 3~5분을 넘는 공간 복잡도 높은 과제에서 흔들린다는 것이다.
...
- π0.6 해설 ([[physical-ai/jo-2026-pi-0-6-vla-primer]])과 SmolVLA 해설 ([[physical-ai/jo-2026-smolvla-vla-primer]]): 같은 연재의 이웃 글. flow matching과 action expert 설명이 두 글에 먼저 나온다
...
| 용어 | 뜻 |
|---|---|
| 혼합 설계 / 분리 설계 | 글쓴이가 논문의 Unified Design과 Decoupled Design을 옮긴 표기. 이 wiki 페이지들은 통합 설계, 분리 설계로 적는다 |
| weight drift | 글쓴이는 이 용어를 한글 번역어로 옮겨 적었다. 논문 원어이자 용어집 canonical 표기는 weight drift다 |
| 비구체화(disembodied) | 물리적 실체 없이 텍스트와 이미지만 다루는 상태를 가리키는 글쓴이의 표현 |
...
|---|---|---|---|
| fig01 | 통합 설계와 분리 설계 비교 | fetched | (논문 Figure 2와 동일, 논문 페이지에서 임베드) |
