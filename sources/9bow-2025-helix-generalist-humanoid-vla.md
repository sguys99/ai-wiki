...
원문 요약이라 서술은 얕다. S2가 초당 7회에서 9회 동작하면서 인터넷에서 pre-training된 VLM으로 장면과 명령을 해석하고, S1이 초당 200회 동작하면서 S2가 만든 의미 표현을 받아 실시간으로 움직임을 조정한다는 두 문단이 아키텍처 설명의 전부다. S2가 고수준 목표를 정하고 S1이 실시간 동작을 수행하는 분업 덕에 정확한 동작을 빠르게 낼 수 있다고 정리한다.
...
원문과 마찬가지로 정량 결과가 없다. 다섯 가지 특징을 나열하는 형식이다. 상반신 전체 제어, 두 로봇의 협업, 처음 보는 물건 수천 개를 즉시 다루는 범용 조작, 과제별 fine-tuning 없이 하나의 가중치 묶음으로 동작하는 단일 신경망, 그리고 저전력 임베디드 GPU에서 동작하는 상용 준비 상태다.

한 대목에서 원문보다 강한 표현을 쓴다. 상용 준비를 설명하며 "추가적인 하드웨어 비용 없이 바로 상업적 배포가 가능합니다"라고 쓰는데, 원문은 저전력 임베디드 GPU에서 온보드로 동작한다는 사실과 그래서 상용 배치 준비가 됐다는 서술까지만 하고 하드웨어 비용에 대해서는 아무 말도 하지 않는다. 저자가 밝힌 GPT 정리 과정에서 덧붙은 서술로 보인다.
...
| fig02 | System 2 → latent 벡터 → System 1 구조도 | fetched | ★ wiki 권장 (원문에서 수집하지 못한 핵심 도식) |
