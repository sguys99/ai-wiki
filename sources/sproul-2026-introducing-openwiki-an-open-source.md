...
LangChain의 Brace Sproul이 2026년 7월 1일 OpenWiki 출시를 알린 공식 소개 글이다. 핵심 주장은 하나다 — 에이전트는 자기가 만지는 저장소를 이해할 때 코드를 더 잘 쓴다. 그런데 문서는 손으로 유지하기 어려워 금세 낡는다. OpenWiki는 저장소 위키를 자동으로 만들고, 그 위키를 코딩 에이전트에 연결한 뒤, 코드가 바뀔 때마다 diff로 갱신해 이 간극을 메운다. 글은 왜 위키인지, 지시 파일과 어떻게 분업하는지, GitHub Action으로 어떻게 최신 상태를 유지하는지를 차례로 설명한다.
...
1. **문제 정의를 또렷이 한다.** 초기 문서 작성에도 시간이 들고, 코드가 바뀔 때마다 갱신하는 일은 더 어렵다. PR이 잦은 대형 저장소일수록 문서가 빠르게 뒤처진다 — 이 낡음 문제가 OpenWiki가 겨냥하는 지점이다.
2. **"왜 위키인가"를 컨텍스트 관리로 답한다.** 대부분의 코딩 에이전트는 이미 `AGENTS.md`·`CLAUDE.md`를 읽는다. 하지만 수백 페이지의 저장소 문서를 그 파일에 다 넣을 수는 없다. 지시 파일은 올바른 맥락을 가리키는 포인터여야 하고, 실제 검색은 에이전트에게 맡겨야 한다.
...
4. **diff 기반 백그라운드 갱신.** GitHub Action을 하루 한 번 같은 주기로 돌리면, 지난 실행 이후의 커밋을 확인하고 git diff로 변경을 이해한 뒤 관련 맥락만 위키에 반영한다. 에이전트는 지시 파일의 참조를 통해 늘 최신 위키를 집어 든다.

## 3. 방법론 및 아키텍처 (Methodology and Architecture)

글이 그리는 흐름은 생성 → 연동 → 갱신의 3단이다.

- **생성**: `npm install -g openwiki` 후 `openwiki --init`. init이 모델 provider와 API 키를 물은 뒤 저장소 문서를 만든다. 기본값은 OpenRouter의 오픈 모델이지만 OpenRouter·Fireworks·Baseten·OpenAI·Anthropic 중 원하는 provider로 바꿀 수 있다.
...
- 소개 글이라 정량 벤치마크는 없다. 근거는 "에이전트가 저장소를 이해할수록 더 정확한 코드 변경을 하고 피할 수 있는 실수를 줄인다"는 정성적 논지다.
