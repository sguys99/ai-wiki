...
PyTorch Korea User Group 운영자 9bow(박정환)가 2026-05-05에 게시한 RAG-Anything 한국어 소개글이다. 홍콩대학교 데이터 인텔리전스 랩(HKUDS)이 공개한 멀티모달 RAG 프레임워크를 도전 과제, 핵심 기여, 5단계 파이프라인, 벤치마크 결과, ablation, 설치와 사용법 순으로 정리한다. 글은 이중 그래프 구축과 크로스-모달 hybrid search를 핵심으로 꼽고, 100페이지를 넘는 긴 문서에서 MMGraphRAG와의 격차가 13점 이상으로 벌어진다는 점을 강조한다. 글 말미에 명시된 대로 본문은 GPT 모델로 정리된 2차 자료이므로, 정밀한 인용은 원 논문을 우선한다.
...
1. **이중 그래프 구축**: 크로스-모달 지식 그래프와 텍스트 기반 지식 그래프를 별도로 구축한 뒤 entity 정렬로 융합하는 전략을 쓴다.
2. **크로스-모달 hybrid search**: 그래프의 구조적 탐색과 임베딩 공간의 의미 유사도 매칭을 결합해 multi-hop 추론과 의미 검색을 동시에 수행한다.
...
| 2 | 크로스-모달 지식 그래프 구축 | 각 콘텐츠 단위에서 검색용 설명과 그래프 구축용 entity 요약을 생성하며 문맥을 고려한다 |
| 3 | 그래프 융합 및 인덱스 생성 | 멀티모달 그래프와 텍스트 기반 지식 그래프를 entity 이름으로 정렬하여 통합한다 |
| 4 | 크로스-모달 hybrid search | 구조적 탐색과 의미 유사도 매칭을 병행하여 후보를 검색하고 다시 순서를 매긴다 |
...
- **베이스라인 수치 미수록**: DocBench와 MMLongBench 표에 RAG-Anything 행만 실려 있어, 전체 순위나 두 번째로 높은 방법과의 차이는 이 글만으로 확인할 수 없다. ablation 표와 긴 문서 비교 표에서만 다른 방법의 수치가 나온다.
...
- **이중 그래프 구축**: 크로스-모달 지식 그래프와 텍스트 기반 지식 그래프를 따로 만든 뒤 entity 정렬로 합치는 전략.
- **entity 정렬**: 두 지식 그래프를 entity 이름 기준으로 맞춰 하나의 인덱스로 통합하는 절차.
- **크로스-모달 hybrid search**: 그래프의 구조적 탐색과 임베딩 공간의 의미 유사도 매칭을 함께 수행하는 검색 방식.
