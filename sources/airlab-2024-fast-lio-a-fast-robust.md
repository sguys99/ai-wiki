...
odometry를 시작 지점부터 현재까지의 상대 pose로 정의한다. 어떤 센서를 쓰느냐에 따라 VO, LIO 등으로 이름이 나뉜다. 그 구분까지 정리한다. FAST-LIO의 특징으로는 solid-state LiDAR 사용, IMU 비중이 큰 tightly-coupled 융합, 새 Kalman gain 계산을 꼽는다.
...
Kalman gain 공식만 놓고 비교하면 개선 공식이 "작게는 100에서" 빨랐다고 말한다 (10:21). 상한 값은 자동 자막이 "만게는 거의 1분까지"로 옮겨 판독되지 않으므로 확인할 수 없다. 드론에 실은 실험에서는 32m를 이동해 오차 0.08m, drift 0.3% 미만이 나왔다 (10:30).
...
12분 발표라 수식 유도(⊞/⊟ 연산, Jacobian 전개)는 슬라이드에 띄우되 자세히 다루지 않는다. 유도가 필요하면 [[physical-ai/taeyoung-2022-fast-lio-paper-review]]가 낫다. 수집 원본이 자동 자막이어서 세부 표현은 신뢰도가 낮다. 수치와 주장을 인용할 때는 논문 페이지에서 재확인한다.
...
- front-end / back-end: SLAM 파이프라인에서 센서 데이터 해석과 feature 대응을 맡는 앞단과, state 추정 및 map 추정을 맡는 뒷단
