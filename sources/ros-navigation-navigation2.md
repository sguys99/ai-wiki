...
- 라이선스: 패키지별 혼합이다. LGPL-2.1-or-later와 Apache-2.0이 섞여 있다. 코드를 인용하거나 재사용할 때는 해당 패키지의 라이선스를 개별 확인해야 한다
...
- 빌드 상태 표가 사실상의 패키지 카탈로그다. humble, jazzy, lyrical 배포판별로 각 패키지의 소스와 데비안 빌드 상태를 나열한다.
...
| 묶음 | 패키지 |
|---|---|
| 조율과 수명 관리 | nav2_bt_navigator, nav2_behavior_tree, nav2_lifecycle_manager, nav2_bringup |
...
| 안전과 후처리 | nav2_collision_monitor, nav2_velocity_smoother, nav2_smoother, nav2_constrained_smoother |
| 복구와 미션 | nav2_behaviors, nav2_waypoint_follower |
| 도킹과 추종 | nav2_docking(opennav_docking 계열), nav2_following |
...
README 자체에는 벤치마크가 없다. 속도와 경로 품질 같은 성능 주장은 인용 논문 쪽에 있다. 특히 Marathon 2와 Smac Planner 논문이다.
...
- **Regulated Pure Pursuit (RPP)**: 곡률과 충돌 위험에 따라 속도를 조절하는 pure pursuit 변형 controller
...
repo 내 이미지는 로고와 스폰서뿐이라 wiki 임베드 후보가 없다. 아키텍처 도식은 [[physical-ai/yhoons-2024-ros2-nav2-intro]] fig02를 쓴다.
