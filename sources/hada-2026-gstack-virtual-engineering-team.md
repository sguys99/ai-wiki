...
- **귀속**: gstack을 "YC CEO Garry Tan이 만든 오픈소스 소프트웨어 팩토리"로 소개한다 <!-- lint-terms: ignore 한국어 원문 직접 인용 -->
- **라이선스**: MIT

## 2. 주요 기여 (Key Contributions)

1. gstack을 개인이 AI로 20인 팀처럼 일하게 해주는 오픈소스 software factory로 요약한다.
2. `Think → Plan → Build → Review → Test → Ship → Reflect` 전체 스프린트 사이클을 슬래시 커맨드가 덮는다는 점을 구조로 제시한다.
...
- 파워 툴의 Codex 리뷰를 슬래시 커맨드 형태로 적지 않는다. `/codex`라는 표기는 이 자료에 없다.
