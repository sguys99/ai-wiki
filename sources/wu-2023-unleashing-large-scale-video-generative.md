...
figures:
  ...
  - id: fig03
    ...
    caption: "CALVIN ABCD→D 실행 장면. 슬라이딩 도어와 서랍, 블록, LED를 다루는 지시문 6종"
    page: 6
  ...
  - id: fig09
    ...
    caption: "실제 기기 rollout. 앞 다섯 줄이 object transportation, 나머지가 서랍 조작 (부록)"
    page: 19
  ...
  - id: fig11
    ...
    caption: "object transportation과 서랍 조작의 영상 예측 결과 (부록)"
    page: 21
  ...
  - id: tab03
    ...
    caption: "pre-training과 fine-tuning 하이퍼파라미터 (부록)"
    page: 14
  ...
  - id: tab04
    ...
    caption: "ablation 결과. pre-training과 영상 예측을 하나씩 뺐을 때 세 설정의 성적 변화"
    page: 15
...
ByteDance Research의 GR 시리즈 첫 편이다. 모델 이름 GR-1은 이후 GR-2와 GR-3로 이어진다.
...
- 시뮬레이션과 실제 기기 양쪽에서 데이터 양, 환경 변화, 지시문 변화를 바꿔가며 폭넓게 검증했다.
...
출력을 뽑아내는 자리로 학습 가능한 토큰 두 종류를 둔다. `[ACT]`는 arm과 그리퍼 action을, `[OBS]`는 미래 프레임을 담당한다. 토큰 시퀀스는 단계마다 이렇게 배열된다.
...
causal Transformer는 12층, 12헤드, hidden 384 구성이고 전체 195M 파라미터 중 46M만 학습된다. 나머지는 얼려둔 CLIP과 MAE ViT다. AdamW에 cosine decay를 걸었고, pre-training은 batch 1024, lr 3.6e-4, 50 epoch이고 fine-tuning은 batch 512, lr 1e-3, 20 epoch다. 이미지에는 random shift augmentation을 적용한다.

## 4. 주요 결과와 벤치마크 (Key Results and Benchmarks)

### CALVIN

CALVIN은 Franka Emika Panda와 평행 그리퍼, 슬라이딩 도어와 서랍, 색깔 블록, LED, 전구가 놓인 책상으로 이뤄진 벤치마크다. 과제는 34개이고 환경은 책상 색과 물체 배치가 다른 A~D 넷이다. 평가는 지시문 5개를 연달아 주고 한 번에 하나씩 통과해야 다음으로 넘어가는 방식이며, 1,000개 시퀀스를 실행한다. 과제당 360 timestep 안에 못 끝내면 실패다.
...
Avg. Len.은 5개 연속 시도에서 평균 몇 개를 통과했는지를 재는 값이라, 긴 과제 사슬을 얼마나 유지하는지를 한 숫자로 보여준다. 격차가 가장 큰 쪽은 ABC→D다. C까지만 학습하고 처음 보는 D에서 평가하는 설정인데, 최고 baseline이 0.90인 자리에서 GR-1은 3.06을 낸다. 저자들은 Ego4D의 다양한 사람-물체 상호작용 덕분에 환경이 바뀌어도 시각-언어 대응이 흔들리지 않는다고 본다.
...
7-DoF Kinova Gen2에 RealSense를 end-effector에, Kinect Azure를 고정 시점에 달았다. object transportation은 가지와 브로콜리, 파프리카를 접시와 책상 사이로 옮기는 과제이고, HTC Vive VR로 teleoperation해 시연 데이터(demonstration) 1,775개를 모았다. teleoperation은 사람이 로봇을 원격으로 움직여 시연 데이터를 만드는 방식이다. 서랍 여닫기는 2,856개를 모았다.
...
두 요소가 각각 기여하며, 데이터가 적을수록 pre-training의 몫이 커진다. 10% 설정에서 1.04 → 1.52 → 2.00으로 거의 두 배가 된다. 다만 영상 예측만 붙이고 pre-training을 뺀 변형은 실제 기기에서는 오히려 아무것도 안 붙인 변형보다 낮은 성공률을 냈다. 예측 능력 자체가 pre-training 없이는 충분히 여물지 않는다는 뜻이다.
...
덧붙일 만한 한계는 평가 범위다. 실제 기기 실험이 단일 팔 로봇 한 대로 한정되어 있고, 과제도 옮기기와 서랍 여닫기 두 종류다. cross-embodiment 전이는 다루지 않는다.

## 6. 관련 연구 (Related Work)

언어 조건부 manipulation 계열에서는 RT-1과 RT-2, CLIPort, PerAct, Hiveformer, 그리고 CALVIN 위에서 경쟁하는 MCIL과 HULC를 비교 대상으로 삼는다. 논문은 앞의 몇 방법이 sparse한 end-effector keypoint를 예측하고 motion planner에 의존한다는 점에서 연속 action을 직접 내는 자신들과 다르다고 선을 긋는다.

Transformer로 순차 의사결정을 다루는 계열에서는 Decision Transformer와 VIMA, GATO, RoboCat을 든다. 특히 RoboCat은 action과 미래 이미지를 함께 예측한다는 점에서 가장 가깝지만, 영상 pre-training이 없고 언어가 아니라 목표 이미지로 조건을 준다.

로봇 학습용 pre-training 계열에서는 R3M과 MVP 같은 표현 학습, VPT와 VIPER처럼 영상을 쓰는 방법을 짚는다. VPT와 VIPER의 영상은 과제 도메인 안의 데이터인 반면 GR-1은 로봇과 무관한 사람 영상을 쓴다. 비교 실험의 MT-R3M이 이 대비를 직접 재는 자리다. 같은 Ego4D로 pre-training했고 학습 파라미터 수도 맞췄지만, 표현만 가져오는 방식이라 GR-1에 크게 밀린다.
...
|---|---|---|---|---|
| fig01 | 2 | GR-1 전체 흐름. pre-training에서 가중치를 복사해 CALVIN과 실제 기기 fine-tuning으로 | caption-region | ★ wiki 권장 (overview) |
| fig02 | 4 | 인코더와 디코더 다섯 구성요소 | caption-region | ★ wiki 권장 (architecture) |
| fig03 | 6 | CALVIN 실행 장면 6종 | caption-region | (확인 필요) |
| fig04 | 7 | CALVIN 네 환경 A~D의 차이 | manual | ★ wiki 권장 (ABC→D 설정 설명용) |
| fig05 | 8 | 실제 기기 실험 설정 네 조건 | caption-region | ★ wiki 권장 (experiment) |
