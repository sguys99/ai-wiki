...
| Block 직원이다 | 소스 빌드도 OSS 릴리스도 쓰지 말고 내부 빌드를 쓰라고 안내한다. 사내 relay와 에이전트 provider에 미리 연결돼 있어 설정할 것이 없고, `squareup/buzz-releases`의 릴리스에서 받는다 |
...
OSS 배포판과 사내 배포판이 갈려 있는 점도 읽을 때 감안해야 한다. Block 내부는 사내 relay와 에이전트 provider에 미리 연결된 빌드를 쓰므로, 실전에서 검증된 구성과 외부에서 세울 수 있는 구성이 같지 않을 수 있다.
