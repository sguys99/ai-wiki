...
문서는 일반적인 LLM 문서 워크플로를 RAG로 규정한다. 파일을 업로드하면 chunk 단위로 vector database에 저장하고, 질의 시점에 관련 chunk를 검색해 답변을 만든다. NotebookLM, ChatGPT 파일 업로드, 대부분의 RAG 시스템이 이 구조라고 본다.
...
LLM Wiki는 LLM이 단순 인덱싱이 아니라 점진적으로 축적되고 발전하는 위키를 구축하고 유지하게 한다. 위키를 사람이 직접 쓰지 않고 LLM이 쓰고 관리한다. 새 소스가 도착하면 LLM은 소스를 읽고 핵심 정보를 추출하며, 기존 위키에 통합해 entity와 개념 페이지를 갱신하고, 새 데이터가 기존 주장과 모순되면 표시하며, 진화하는 종합 분석을 강화하거나 도전한다. 지식은 한 번 컴파일되고 최신 상태로 유지되며 질의마다 재도출되지 않는다.

핵심 통찰은 위키가 영속적이고 compounding하는 산출물(persistent, compounding artifact)이라는 것이다. 교차 참조가 이미 구성돼 있고, 모순점이 이미 표시돼 있으며, 종합 분석이 이미 모든 자료를 반영한다. 소스를 추가할수록, 질문할수록 더 풍부해진다.
...
| Wiki | LLM | LLM 전적 소유 | 마크다운 파일 디렉토리. 요약, entity, 개념, 비교, 종합 분석 |
...
Wiki는 LLM이 생성한 마크다운 파일 디렉토리로 요약, entity 페이지, 개념 페이지, 비교 자료, 개요, 종합 분석을 포함한다. 새 원본 자료가 도착하면 LLM이 갱신하면서 상호 참조를 관리하고 내용을 일관되게 유지한다. 사용자는 읽기만 한다.
...
| 작업 | 시점 | 절차 | 산출 |
|---|---|---|---|
| Ingest | 새 소스 도착 시 | 소스 읽기, 핵심 요점 사용자와 논의, 위키 요약 페이지 작성, 인덱스 갱신, 관련 entity와 개념 페이지 갱신, 로그에 항목 추가 | 단일 소스가 10~15개 위키 페이지에 영향 |
...
Query의 중요한 통찰은 좋은 답변을 위키에 새 페이지로 다시 저장할 수 있다는 점이다. 비교 분석과 발견한 연결고리, 심층 분석이 채팅 히스토리로 사라지지 않고 탐색 활동 자체가 지식 베이스에 compounding 방식으로 축적된다.
...
`index.md`는 콘텐츠 중심 카탈로그다. 위키의 모든 페이지를 링크와 한 줄 요약, 메타데이터(날짜, 소스 수 등)로 카탈로그화하고 entity, 개념, 소스 같은 카테고리별로 정리한다. LLM은 수집할 때마다 갱신하고 질의할 때마다 먼저 읽는다. 소스 약 100개, 페이지 수백 개 규모까지는 임베딩 기반 RAG 인프라 없이 잘 작동한다.
...
| Dataview | 페이지 frontmatter에 질의를 실행하는 플러그인. LLM이 태그, 날짜, 출처 수를 frontmatter에 넣으면 동적 테이블과 목록을 생성한다 |
...
| 번호 | 항목 | 요지 |
|---|---|---|
| 9.1 | 파일명 컨벤션 | 파일명은 영어 kebab-case, 제목(H1)만 한국어. 한글 파일명은 URL 인코딩과 git 호환성 문제를 일으킨다. frontmatter에는 한영 태그를 병기해 Dataview 질의에서 양쪽이 잡히게 한다 |
...
문서 10장은 이 패턴의 위치를 검색 기술의 새 패러다임이 아니라 지식 관리 워크플로의 재구성으로 규정한다. nashsu의 4-Signal Relevance Model과 4-phase Query Pipeline은 결국 그래프 RAG에 가까운 것으로 본다. "RAG가 아니다"라는 프레이밍을 곧이곧대로 받지 말고 hybrid search와 사전 컴파일된 지식 레이어의 조합으로 이해하기를 권한다.
...
기존 RAG 기법과의 연결에서는 GraphRAG, multi-hop 검색, 계층적 요약, hybrid search(BM25와 벡터)이 자연스럽게 이어진다고 보고, 기존 도구로도 충분히 구현할 수 있으며 Karpathy의 기여는 워크플로와 철학 측면이라고 정리한다.
...
- **2-step Chain-of-Thought Ingest**: Step 1(Analysis)에서 소스를 구조화된 분석(entity, 개념, 모순, 추천)으로 바꾸고, Step 2(Generation)에서 분석을 위키 파일로 만든다.
...
| 번호 | 비판 | 반론 또는 평가 |
|---|---|---|
| 7.1 | 결국 RAG다. vector database 대신 인덱스 파일과 파일시스템 계층을 쓸 뿐, 의미적 연결 인덱스와 검색 보조 구조 자체는 RAG와 같다 | 사전 컴파일과 런타임 조립의 차이이며, 검색 기술이 아니라 지식 관리 패턴의 차이라고 반박한다 |
...
- `[[database/zhang-2025-pageindex-vectorless-reasoning-rag]]`, `[[database/kalane-2026-pageindex-threw-out-vector-databases]]`. vector database 없이 목차를 컨텍스트 안에 두는 retrieval. 임베딩 없는 `index.md` 발상과 통한다.
...
- **Ingest**: 새 소스를 원시 컬렉션에 추가하고 LLM이 요약, 인덱스, entity 페이지, 로그까지 갱신하는 작업. 단일 소스가 위키 페이지 10~15개에 영향을 준다.
