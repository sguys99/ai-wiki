...
task 번들 하나는 지시 파일(instruction file), reward 파일, 초기 setup 파일로 이루어진다.
...
task 하나를 실행하는 절차는 다섯 단계다. artifact 아카이브를 풀고, `<task_id>/task.json`을 읽고, 명시된 setup 단계를 대상 환경에서 실행하고, 에이전트가 환경과 상호작용하게 두고, 마지막으로 `<task_id>/reward.py`를 실행해 프로그램 방식 점수를 계산한다.
...
데이터셋 배포는 뷰어용 테이블과 실행 가능한 artifact를 분리한 네 항목 구조다.
...
인용 요구는 별도 절로 적혀 있다. 코드, 도구, Hub 환경, 데이터셋, 모델, 생성된 task artifact 중 어느 것이든 공개 자료에 쓰면 명시적 사사와 논문 인용을 포함해 달라고 요청한다. 상표 사용이 보증이나 제휴를 뜻하지 않는다는 조항도 있다.
