...
π0.5 논문과 같은 날 올라온 Physical Intelligence 공식 발표문. 수식 없이 co-training 레시피를 설명하고 논문이 막대그래프로만 보여 준 ablation 수치를 숫자로 적어 두었다. 새 집에서 촬영한 무편집 데모 영상도 함께 싣는다.
...
논문에는 그림으로만 있는 언어 지시 따르기 ablation의 수치가 본문에 적혀 있다. in-distribution과 out-of-distribution 각각의 follow rate와 success rate를 조건별로 읽을 수 있다.
...
마지막은 무편집 데모다. 새 집에서 촬영한 long-horizon 과제 영상 7편, 사람이 방해했을 때의 반응, 언어 따르기 영상이 실렸다. 여기서는 지시문 세분화 정도를 바꿔 가며 물건을 집게 했다. 논문 Figure 2와 7이 정지 프레임으로 보여 주는 장면의 원본이다.
...
다른 로봇에서 온 데이터(ME와 CE)는 모든 조건에서 성능을 좌우한다. 웹 데이터(WD)의 효과는 학습에서 못 본 물체 범주로 갔을 때만 크게 드러난다. 읽는 방법도 이렇게 나란히 적어 두었다.
...
자사 자료로는 π0 블로그(직전 세대 VLA), FAST 연구 페이지(π0-FAST의 action tokenizer), Hi Robot(high-level과 low-level 분리 구조)을 본문에서 링크로 건다. 논문 PDF도 `/download/pi05.pdf`로 함께 건다.
