...
이 글은 PyTorch KR 커뮤니티의 9bow(박정환)가 Garry Tan의 gstack을 한국어로 정리한 소개 글이다. gstack은 Y Combinator 대표가 공개한 오픈소스 software factory로, 글은 이를 "Claude Code를 사용하는 개발자가 20인 팀처럼 출시할 수 있도록" 설계된 28가지 슬래시 커맨드 모음으로 소개한다.
...
읽는 목적을 기준으로 보면, 이 글은 gstack의 내부 구현을 파고드는 자료가 아니라 어떤 명령어가 있고 무엇을 하는지 빠르게 확인하는 인덱스에 가깝다. 저장소 원문 정리는 [[agents/garrytan-gstack]]이, 브라우저 데몬 같은 내부 동작은 [[agents/gpters-2026-yc-ai-agent-guide-gstack]]이 맡는다.

## 배경

### 하나의 모드로만 응답하는 어시스턴트

gstack이 겨냥하는 문제는 모델의 코딩 실력이 아니라 작업 절차의 부재다. 이 문제 인식은 이 글이 아니라 GPTERS 해설([[agents/gpters-2026-yc-ai-agent-guide-gstack]])의 것이다. 그 해설은 Claude Code가 무엇을 요청하든 한 가지 모드로만 응답해, 코드를 부탁하면 곧바로 작성에 들어갈 뿐 이 기능을 만드는 것이 맞는지 먼저 묻지 않는다고 적는다.

같은 해설이 드는 대비에 따르면 실제 조직에서는 이 판단이 여러 사람에게 나뉘어 있다. 방향은 CEO가 정하고, 설계는 엔지니어링 매니저가 잠그고, 리뷰는 시니어 엔지니어가 하고, 검증은 QA가 맡는다. 혼자 쓰는 에이전트에는 이 분업이 없다.

gstack은 그 분업을 슬래시 커맨드 단위로 되살린다. 글이 첫머리에서 열거하는 역할은 CEO, 디자이너, 엔지니어링 매니저, QA 리드, 보안 담당자, 릴리즈 엔지니어다. 명령어 하나를 부르면 그 역할의 관점으로 AI가 작업을 수행한다는 구성이다.
...
**software factory**는 반복 가능한 파이프라인으로 소프트웨어를 생산하는 체계를 뜻한다. 글은 gstack을 오픈소스 software factory로 소개하는데, 판매하는 것이 코드 생성 능력이 아니라 생산 절차라는 의미다.

**슬래시 커맨드**는 Claude Code에서 `/이름` 형태로 특정 역할이나 작업을 호출하는 단위다. gstack에서는 명령어 하나가 곧 전문가 한 명에 대응하므로, 명령어를 고르는 행위가 곧 어떤 관점으로 검토받을지를 고르는 행위가 된다.
...
글은 gstack을 28가지 슬래시 커맨드 모음으로 소개하지만, 본문에 이름이 실제로 나오는 명령어는 22개다. 그룹별로 5개, 3개, 4개, 3개, 7개를 합한 값이다. 나머지 6개가 무엇인지는 글에 나오지 않으므로, 전체 명령어 목록으로 삼기에는 완결되지 않는다.
...
같은 도구를 다루지만 이 글과 GPTERS 해설([[agents/gpters-2026-yc-ai-agent-guide-gstack]])이 다르게 적는 항목이 있다. 아래 오른쪽 열의 값은 원 저장소 README가 아니라 그 해설의 것이다. 어느 쪽이 최신인지는 두 자료만으로 판단할 수 없으므로, 실제 사용 시점에는 저장소 문서를 확인하는 편이 안전하다.

| 항목 | 이 글의 서술 | [[agents/gpters-2026-yc-ai-agent-guide-gstack]]의 서술 |
|---|---|---|
...
| 슬래시 커맨드 | Claude Code에서 `/이름` 형태로 특정 역할이나 작업을 호출하는 단위. gstack에서는 명령어 하나가 역할 하나에 대응한다 |
