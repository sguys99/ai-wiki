...
점수는 에이전트가 필요로 할 가능성이 높은 행이 위로 올라오게 설계됐다. 활성 질의(query)나 호출자가 지정한 키워드에 걸리면 점수가 오르고, 오류처럼 보이는 행은 더 오른다.

| 조건 | 가산점 | 비고 |
|---|---:|---|
| 활성 질의와 일치 | +0.3 | 검색어가 행 내용에 등장 |
| 호출자 지정 키워드와 일치 | +0.4 | 호출 측이 넘긴 관심 키워드 |
| 오류 패턴과 일치 | +0.5에서 0.1씩 감소 | error, warning, 일반 중요도 순으로 검사하고 첫 일치에서 멈춘다 |

오류 부스트가 한 행에 한 번만 적용되므로 오류 행이 두 번 가산되는 일은 없다. 그럼에도 오류 행의 가산점이 단순 질의 일치보다 크기 때문에, 같은 질의에 걸린 행 중에서는 오류를 담은 행이 먼저 살아남는다.
...
### caveman이 프롬프트를 주입하는 방식

caveman은 앞의 두 도구와 접근이 다르다. 런타임에 페이로드를 변환하지 않고, 프롬프트를 주입해 모델의 발화 습관 자체를 바꾼다. 압축 대상이 데이터가 아니라 모델의 문체다.
