...
`sid` 질의(query) 파라미터가 session isolation의 실제 구현이다. 설계 원리로는 세션 격리라 부르지만 인터페이스는 질의 문자열 하나로 끝난다.
...
데이터셋은 브라우징용 테이블과 실행 가능한 artifact를 분리한 구조다.
...
인용 요구도 별도 절로 적혀 있다. 코드, 도구, Hub 환경, 데이터셋, 모델, 생성된 task artifact 중 어느 것이든 공개 자료에 쓰면 명시적 사사와 논문 인용을 포함해 달라고 요청한다. 면책 조항은 배포와 샌드박스 구성, 안전한 운영의 책임을 사용자에게 둔다고 밝히고, 상표 사용이 보증이나 제휴를 뜻하지 않는다는 조항도 함께 둔다.
