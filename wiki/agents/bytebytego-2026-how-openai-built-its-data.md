...
| 구성 요소 | 구현 | 역할 |
|---|---|---|
| LLM | GPT-5.5 단일 모델 | 올바른 SQL 생성, 결과 검사, 질의 수정, 검증된 답까지의 추론 |
...
LLM은 모든 요청에 같은 모델을 쓴다. 팀은 GPT-5.5가 올바른 SQL을 만들고, 결과를 살펴보고, 필요하면 질의를 고치고, 검증된 답에 이를 때까지 추론하리라고 기대한다.
...
첫 layer의 핵심은 모든 질의가 똑같이 유용하지는 않다는 판단이다. 데이터 사이언티스트가 작성해 인기 대시보드를 떠받치는 질의는 정확하고 재사용될 가능성이 높아 가장 높은 순위를 받는다. 반대로 한 번 쓰고 마는 탐색용 질의는 낮은 순위로 내려간다.

두 번째 layer는 스키마나 과거 질의에서 추론할 수 없는 정보를 담당한다. 테이블 소유자가 직접 쓰는 설명이며, 업무상 무엇을 뜻하는지, 누가 소유하는지, 얼마나 중요한지, 어떤 함정이 알려져 있는지를 적는다.
...
패치가 하나 추가될 때마다 세 단계를 거쳐야 한다. 기존 테스트 스위트로 검증하고, 스테이징에서 확인하고, 프로덕션으로 rollout한다. 플랫폼 신뢰성에 중요한 작업이지만 반복적이고 시간이 많이 든다. 테스트 스위트가 길어서 몇 시간짜리도 있고 며칠씩 이어지는 것도 있다.
...
| 오픈소스 패치 릴리스 | 검증, 실패 진단, 프로덕션 rollout, 보고 | 알림 확인 | 기존 테스트 스위트와 스테이징 단계 |
...
| 신뢰할 수 있는 질의만 검색 대상으로 | 과거 질의 전량 임베딩은 실패했다 |
...
### 신뢰할 수 있는 질의만 검색 대상으로

data agent를 만들 때 자연스러운 첫 발상은 과거 질의를 전부 임베딩해 컨텍스트로 쓰는 것이다. OpenAI도 시도했고 잘 되지 않았다. 어느 회사든 대부분의 질의는 일회성 탐색이지, 테이블을 어떻게 써야 하는지 보여 주는 정본 예시가 아니기 때문이다.

팀은 과거 질의에 신뢰도 순위를 매겨 결과를 개선했다. 순위 기준은 다음과 같다.

| 질의 유형 | 순위 | 이유 |
|---|---|---|
| 많이 쓰이는 대시보드를 떠받치는 질의 | 최상위 | 보통 데이터 사이언티스트가 작성하며 정확하고 자주 재사용된다 |
| 단일 분석용으로 쓰고 다시 실행되지 않는 질의 | 최하위 | 정본 예시로 삼기 어렵다 |
