...
| Observability | 로그, trace, 평가, 비용과 지연 계측. 없으면 agent가 잘하고 있는지 조용히 드리프트하고 있는지 알 방법이 없다 |
...
2. 규모화 전에 프로덕션 기반(substrate)에 투자한다. 노트북의 vibe coding 프로토타입은 프로덕션 시스템이 아니다. 하나를 다른 하나로 졸업시키는 것은 주변의 운영 규율이다. CI에서 실행되는 trajectory와 최종 응답 eval, 모든 agent 실행의 trace, agent별 범위 지정 권한, 생성 코드의 실패 모드에 맞춰 조정된 보안 리뷰다. 첫 프로덕션 agent가 배포된 뒤가 아니라 그 전에 이 기반을 만든다.
