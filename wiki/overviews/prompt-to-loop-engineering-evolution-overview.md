...
AI 코딩 에이전트를 다루는 방식은 지난 4년 동안 **무엇을 최적화하느냐**의 초점이 한 칸씩 바깥으로 옮겨 왔다. 단일 지시(prompt)에서 문맥 토큰(context)으로, 다시 실행 환경(harness)으로, 그리고 오케스트레이션 루프(loop)로 넘어온 흐름이다. 각 단계는 앞 단계를 버리지 않고 감싼다. 좋은 프롬프트는 지금도 필요하지만, 사람이 매번 손으로 쓰는 대신 루프가 대신 만들어 공급할 뿐이다([[agents/kang-2026-no-longer-prompting-claude]]의 역설).

이 페이지는 그 네 단계 진화를 한눈에 잡는 최상위 진입점이다. 앞의 두 단계인 Prompt와 Context는 배경으로 짧게 짚고, 자료가 두껍게 쌓인 Harness와 Loop 두 단계는 각각 아래의 상세 페이지로 안내한다.
...
> 단계별 시점·제안자 배열은 [[agents/kang-2026-no-longer-prompting-claude]]가 정리한 타임라인을 따랐다. Prompt 단계의 원전 논문(ReAct·CoT 등)은 wiki에 **전용 페이지로 보유하지 않는다**. patel 리스트가 추천 목록으로 언급하는 정도다.
...
- **pre-retrieval → just-in-time**: 미리 다 넣는 대신 식별자(file path·query)만 두고 런타임에 tool로 로드한다. Claude Code가 `head`·`grep`으로 대용량을 훑는 방식이다.
- **long-horizon 3기법**: compaction(요약 후 재출발), structured note-taking(외부 메모리), sub-agent(clean context 위임).

문맥을 다루는 다른 형태의 자료도 이 단계에 속한다. [[database/edge-2024-from-local-to-global]](GraphRAG: 대규모 corpus를 context window의 1/9~1/43로 compaction), [[agents/zhang-2026-recursive-language-models]](context를 REPL **변수**로 두고 sub-LM 재귀 호출), [[applications/datasciencedojo-2026-llm-wiki-by-andrej-karpathy]](RAG 대비 **사전 컴파일된** stateful context)가 그렇다.

## 단계 3 — Harness Engineering: 실행 환경을 설계하다 (심화)

초점이 한 번 더 바깥으로 나간다. 프롬프트도 문맥도 결국 **에이전트가 일할 작업 환경**의 일부다. 가드레일·툴셋·맥락·검증·흐름을 아우르는 그 환경 전체를 설계하는 일이 **harness engineering**이다. [[agents/lee-hoyeon-2026-harness-engineering]]가 이를 가장 넓은 틀로 그린다. *"말을 잘 하는 것에서 일하는 방법을 설계해주는 것으로."*
...
각 축은 Claude Code 도구로 이어진다. 구조(폴더링·Skills·Hooks·MCP), 맥락(CLAUDE.md 3-tier 상속·Progressive Disclosure), 계획(Plan Mode·커스텀 스킬), 실행(Single/Subagent/Team·Ralph Loop), 검증(Generator/Evaluator 분리·Browser Agent), 개선(3회 반복→Skill, 3회 실수→Rule)이 그 예다. [[agents/patel-2026-beyond-the-prompt-claude-code]]는 이 축들을 `.claude/` 디렉토리 config로 구체화한 실전 매뉴얼을 제공한다.

이 단계의 핵심 주장은 **"모델보다 하네스가 큰 lever"**라는 것이다. 이호연이 인용한 수치를 보면, LangChain은 같은 모델에서 하네스만 바꿔 TerminalBench 성적을 +14%p 끌어올려 30위에서 Top 5로 올라섰고, Anthropic은 싱글 에이전트로 $9를 쓰고 실패한 작업을 3에이전트 하네스로 $200에 성공시켰으며, Stripe는 주당 1,000건의 PR을 무인 머지한다. 결론은 이렇다. *"모델 교체로 5% 개선보다 하네스 설계로 15% 개선이 현실적이다."*

이 낙관에는 두 가지 제약이 따른다:

- **실증의 경계**: [[agents/lin-2026-harness-updating-is-not-harness-benefit]]는 7 LLM × 3 벤치마크를 controlled grid로 계측해, 하네스 lever가 frontier 모델에서만 제대로 회수된다는 점을 보인다. 약한 모델은 Skill을 불러오는 format조차 못 맞추거나(activation failure), 절차를 끝까지 따르지 못한다(adherence failure).
- **입력단의 경계**: [[agents/trq212-2026-a-field-guide-to-fable]]는 하네스의 **앞단**을 파고든다. 하네스를 아무리 정교하게 깔아도 사람이 원하는 바가 흐릿하면(map≠territory의 unknown) 결과도 흐려진다. 하네스가 model보다 큰 lever라면, 그 하네스에 무엇을 넣느냐가 다시 그 위의 lever인 셈이다.
...
마지막 단계다. 하네스를 다 깔았다면, 이제 그 위에서 **사람 손 없이 도는 루프**를 설계할 차례다. [[agents/osmani-2026-loop-engineering]] (Addy Osmani, 2026-06-07)가 *"prompting agents → designing loops that prompt your agents"* 전환에 **Loop Engineering**이라는 이름을 붙였다.

루프의 최소 골격은 **5+1 구성 요소**다:
...
Claude Code의 슬래시 커맨드 두 개가 루프의 두 유형을 가른다. `/loop`은 **cadence-based**로 시간 주기에 맞춰 돌고, `/goal`은 **conditional completion**으로 조건이 충족될 때까지 grind하며 검증은 별도 모델에 위임한다. [[agents/runkle-2026-the-art-of-loop-engineering]]는 이 루프를 4단계 스택(agent → verification → event-driven → hill climbing)으로 정리했고, [[agents/lee-jeongmin-2026-loop-engineering-claude-code]]는 Claude Code의 dynamic workflow를 **RLM(Recursive Language Model)** 이론에 묶어 그 설계 의도를 뜯어본다.

제목의 역설([[agents/kang-2026-no-longer-prompting-claude]])이 이 단계의 핵심이다. *"더 이상 프롬프팅하지 않는다"*는 말은 프롬프트를 버린다는 뜻이 아니라, 사람이 매번 쓰던 일을 **루프에 맡긴다**는 뜻이다. 초점이 문장 한 줄에서 그 문장을 자동으로 만들어내는 루프 구조로 옮겨갔을 뿐, 좋은 프롬프트의 가치는 그대로다.

자동화가 앗아가는 것도 함께 짚는다. 이른바 **3대 부채**다:

- **Validation debt(검증 부채)**: 루프가 쏟아내는 결과를 사람이 다 검증하지 못한 채 쌓인다.
- **Understanding/Comprehension debt(이해 부채)**: 루프가 대신 처리한 과정을 이해하지 못하고 넘어가면서 쌓인다.
- **Cognitive resistance/Comfortable passivity(인지적 저항·편안한 수동성)**: 위임이 주는 편안함이 사고를 덜 하게 만든다.

Osmani의 결론도 같다. *"leverage point는 옮겨갔지만, 품질의 책임은 여전히 엔지니어에게 고정되어 있다."*
...
단계가 바깥으로 밀려나도 **같은 세 원리가 이름만 바꿔 되풀이된다**. 네 단계가 별개가 아니라 하나의 진화라는 증거다.

1. **Verification distance — 만드는 AI ≠ 검증하는 AI.** Context 단계의 sub-agent 위임, Harness 단계의 Generator/Evaluator 물리적 분리, Loop 단계의 sub-agent verification이 모두 여기에 해당한다. *"자기 작업을 평가하면 mediocre해도 자신 있게 칭찬한다"*는 문제의식이 세 단계를 관통한다.
2. **Progressive disclosure — 다 넣지 말고 필요할 때 꺼낸다.** Context의 "최소 고신호 집합"과 just-in-time 로드가 Harness의 `references/` 분리와 Skill 조건부 활성화로, 다시 Loop의 맥락별 skill 로드로 이어진다.
3. **Compounding — 실수를 규칙으로, 반복을 Skill로.** Context의 structured note-taking(NOTES.md)이 Harness의 "3회 실수→Rule, 3회 반복→Skill"로, 다시 Loop의 persistent state로 쌓인다. 이 wiki의 3-tier 구조(raw→sources→wiki)도 같은 복리 원리 위에 있다.

**네 단계를 관통하는 제약도 하나 있다.** 사다리를 위로 오를수록 frontier 모델에 대한 의존이 커진다. Lin et al.의 실증([[agents/lin-2026-harness-updating-is-not-harness-benefit]])은 harness·loop의 정교한 설계가 약한 모델에서는 아예 작동하지 않는다는 점을 보인다. 상위 단계의 lever는 **강한 모델을 쓸 때 가장 크게 회수된다**.

## 이 지도를 어떻게 쓰나 (Reading Order)

1. **큰 그림부터** → 이 페이지 §요약·§Ladder로 4단계 축을 잡는다.
2. **배경 두 단계는 빠르게** → [[agents/anthropic-2025-effective-context-engineering-for-ai]] (Context 전환의 원점) 하나면 충분하다. Prompt 단계는 개념만 잡아도 된다.
