...
figures:
  - id: fig01
    ...
    caption: "pre-trained WFM과 post-trained WFM의 생성 예시 모음 — diffusion·autoregressive, camera control·manipulation·자율주행 (Figure 1, p.2)"
    page: 2
  ...
  - id: fig04
    ...
    caption: "Cosmos 플랫폼 5개 구성요소 — Video Curator · Tokenizer · pre-trained WFM · post-training 샘플 · Guardrail (Figure 4, p.5)"
    page: 5
  ...
  - id: fig05
    ...
    caption: "Video Curator 5단계 — split → filtering → annotation → 클립 DB → dedup → sharding (Figure 5, p.6)"
    page: 6
  ...
  - id: fig11
    ...
    caption: "diffusion WFM(Cosmos-Predict1) 아키텍처 — CV8x8x8 토크나이저 latent에 노이즈를 주고 3D patchify 후 self/cross-attention 블록 N회 (Figure 11, p.20)"
    page: 20
  ...
  - id: fig14
    ...
    caption: "autoregressive WFM(Cosmos-Predict1-Video2World) 아키텍처 — DV8x16x16 discrete 토큰에 T5 cross-attention을 붙인 Llama3 계열 구조 (Figure 14, p.28)"
    page: 28
  ...
  - id: fig15
    ...
    caption: "diffusion decoder 학습 — 같은 영상을 discrete·continuous 두 토크나이저로 이중 토큰화한다 (Figure 15, p.33)"
    page: 33
  ...
  - id: fig21
    ...
    caption: "camera control 정성 비교 — Cosmos vs CamCo, 입력 trajectory와 재추정 trajectory (Figure 21, p.42)"
    page: 42
  ...
  - id: fig24
    ...
    caption: "instruction 기반 영상 예측 사람 평가 — 4개 축에서 VideoLDM-Instruction 대비 선호도 (Figure 24, p.46)"
    page: 46
  ...
  - id: fig25
    ...
    caption: "Cosmos-1X 데이터셋에서의 instruction 기반 영상 예측 샘플 (Figure 25, p.47)"
    page: 47
  ...
  - id: fig30
    ...
    caption: "Cosmos Guardrail 구성 — pre-Guard(키워드 차단 + Aegis) / post-Guard(영상 안전 분류기 + 얼굴 블러) (Figure 30, p.54)"
    page: 54
  ...
  - id: tab05
    ...
    caption: "continuous video 토크나이저 평가 (DAVIS·TokenBench) (Table 5, p.16)"
    page: 16
  ...
  - id: tab06
    ...
    caption: "discrete video 토크나이저 평가 (DAVIS·TokenBench) (Table 6, p.16)"
    page: 16
  ...
  - id: tab09
    ...
    caption: "토크나이저 런타임 성능 — 파라미터 수와 이미지·프레임당 인코딩/디코딩 시간 (Table 9, p.17)"
    page: 17
  ...
  - id: tab10
    ...
    caption: "Cosmos WFM 모델 지도 — diffusion 4종 / autoregressive 4종, 각 계열의 토크나이저와 enhancer (Table 10, p.18)"
    page: 18
  ...
  - id: tab18
    ...
    caption: "autoregressive 모델 실패율 — image 조건 vs 9프레임 video 조건 (Table 18, p.36)"
    page: 36
  ...
  - id: tab22
    ...
    caption: "camera control post-training 정량 비교 — CamCo 대비 pose 성공률·회전/이동 오차·FID/FVD (Table 22, p.43)"
    page: 43
  ...
  - id: tab24
    ...
    caption: "multi-view 주행 영상 생성 평가 — FID/FVD와 multi-view 일관성(TSE·CSE) (Table 24, p.52)"
    page: 52
  ...
  - id: tab25
    ...
    caption: "multi-view 주행 영상의 trajectory 일관성 평가 (TAE·TFE) (Table 25, p.52)"
    page: 52
...
NVIDIA가 Physical AI 개발자에게 world model을 통째로 넘겨주려고 만든 플랫폼 논문이다. world model은 환경의 동역학을 학습해 미래를 예측하는 모델인데, 여기에 foundation을 붙인 world foundation model은 특정 로봇이 아니라 여러 downstream 환경으로 fine-tuning될 것을 전제로 한 범용 모델을 가리킨다. 논문은 WFM이라 줄여 쓴다.

출발점은 Physical AI의 데이터 문제다. 센서로 세상을 보고 액추에이터로 세상을 바꾸는 시스템을 학습시키려면 observation과 action이 번갈아 들어간 데이터가 필요한데, 그 action이 기기와 환경을 망가뜨릴 수 있어 데이터를 늘리기 어렵다. 안전하게 상호작용할 수 있는 물리 세계의 디지털 트윈, 곧 WFM이 오래 기다려온 해법이라는 게 저자들의 진단이다.

논문에 담긴 것은 curation 파이프라인, video 토크나이저, diffusion·autoregressive 두 계열의 pre-trained 모델 8종, post-training 사례 셋, Guardrail이다. 학습에는 H100 10,000장을 3개월 썼고 가중치는 NVIDIA Open Model License로 공개했다.

## 주요 기여 (Key Contributions)

pre-training과 post-training을 나눈 구도가 뼈대다. 대규모 다양한 영상으로 generalist WFM을 먼저 만들고, 목표 환경에서 모은 훨씬 작은 "prompt-영상" 쌍으로 특화 모델을 얻는다. prompt 자리에는 텍스트뿐 아니라 action 명령이나 trajectory가 들어갈 수 있어서, 같은 pre-trained 모델이 자율주행 차량으로도 휴머노이드로도 갈라진다.
...
- Cosmos Tokenizer — causal 구조의 continuous·discrete 토크나이저 제품군. 이미지와 영상을 한 네트워크로 처리한다
- pre-trained WFM 8종에 prompt upsampler와 diffusion decoder를 각각 붙였다
- post-training 사례 셋 — camera control로 3D 공간을 돌아다니기, robot manipulation, 자율주행용 6뷰 동시 생성
...
*Figure 4: 플랫폼 5개 구성요소 (NVIDIA 2025, p.5)*

## 방법론 및 아키텍처 (Methodology and Architecture)

### 문제 설정

WFM은 과거 영상 x(0:t)와 현재 perturbation c(t)를 받아 다음 상태 x(t+1)을 내놓는 함수다. perturbation은 world model이 미래를 예측할 때 함께 받는 현재 입력으로, Physical AI가 취한 action일 수도 무작위 입력일 수도 그것을 적은 텍스트일 수도 있다. 이 추상화 덕에 텍스트 조건 생성과 action 조건 생성이 같은 틀에 들어간다.
...
*Figure 5: Video Curator 5단계 (NVIDIA 2025, p.6)*

split은 shot 전환 기준으로 잘라 2초 미만은 버리고 60초 초과는 다시 쪼갠다. 분할 알고리즘 비교에서는 학습 기반인 TransNetV2와 AutoShot이 색 히스토그램 임계값 방식(PySceneDetect)이나 Panda70M보다 확실히 앞섰다. filtering은 움직임·화질·오버레이 텍스트·영상 유형 네 축으로 거르고, annotation은 VLM이 256프레임마다 캡션을 붙인다. 그 뒤 클립 DB에 넣고 의미 기준 중복 제거를 거쳐 해상도와 종횡비별로 sharding한다. 결과는 pre-training용 약 10^8 클립, fine-tuning용 약 10^7 클립이다.

### 토크나이저

Cosmos Tokenizer는 wavelet 공간에서 동작하고 시간 방향으로 causal이라 현재 프레임 계산에 미래 프레임을 쓰지 않는다. 여기에 이점이 둘 붙는다. 단일 프레임을 넣으면 그대로 이미지 토크나이저가 되므로 이미지와 영상을 함께 학습시킬 수 있고, 인과 순서로 사는 Physical AI 시스템과 성격이 맞는다.
...
두 계열 각각 base 모델 둘과 파생 모델 둘, 여기에 계열별 enhancer 하나가 붙는 대칭 구조다.
...
latent diffusion이고 학습 목표는 EDM의 denoising score matching이다. 노이즈 레벨별 손실을 다중 과제 학습으로 보고, 불확실성 함수 u(σ)를 MLP로 두어 모델이 자신 없어 하는 레벨의 기여를 낮춘다. 네트워크는 DiT를 영상용으로 고친 것으로, latent를 (1, 2, 2) 큐브 단위로 3D patchify한 뒤 self-attention → cross-attention → MLP 블록을 N회 반복하고 adaptive layer norm의 scale·shift·gate로 timestep을 주입한다. 위치 정보는 FPS를 반영한 3D RoPE와 학습되는 절대 위치 임베딩을 함께 쓰고, 텍스트는 T5 인코더 출력이 cross-attention으로 들어간다.

![[assets/nvidia-2025-cosmos-world-foundation-model-platform/fig11.png]]
*Figure 11: diffusion WFM 아키텍처 (NVIDIA 2025, p.20)*

Text2World를 먼저 학습하고 영상 입력을 추가로 받도록 fine-tuning해 Video2World를 얻는다. 학습용 텍스트 prompt는 VLM이 만든 캡션이라 사람이 쓰는 문장과 분포가 다른데, 이 간극은 Mistral-NeMo-12B-Instruct 기반 prompt upsampler가 메운다.

### autoregressive 계열

Llama3 스타일 GPT를 영상 토큰 예측용으로 처음부터 학습시킨 모델이라 언어 이해 능력은 없다. 텍스트를 쓰려면 T5-XXL 임베딩을 self-attention 층마다 붙인 cross-attention으로 넣는다.
...
학습 안정성 장치가 둘 붙는다. QKNorm은 query와 key를 정규화한 뒤 학습되는 γ로 스케일해 softmax 포화를 막고, z-loss는 logit 제곱합에 λ=3×10^-4를 곱해 더한다. 저자들은 노드 수를 크게 늘릴 때 z-loss가 그래디언트 norm 유지에 결정적이었다고 적었다.

학습은 3단계다. 첫 프레임만 주고 16프레임을 예측하는 17프레임 문맥에서 시작해, 시간축 RoPE에 YaRN 확장을 걸어 34프레임으로 늘리고, 마지막에 cross-attention을 새로 초기화해 텍스트 조건을 넣는다. 해상도는 640×1024 고정이고 끝에 고품질 데이터로 학습률을 0까지 선형 감쇠시키는 cooling-down을 30,000 iteration 돌린다. 12B 모델은 파라미터·그래디언트·옵티마이저 상태만 약 192GB라 H100 한 장에 안 들어가므로 tensor parallelism과 sequence parallelism으로 쪼갠다. 추론 쪽은 KV 캐시·torch.compile에 더해 Medusa 방식의 추측 디코딩을 얹었다.

discrete 토크나이저의 압축률이 높다 보니 왜곡이 생기는데, 7B Text2World를 fine-tuning해 DV8×16×16 토큰을 CV8×8×8 공간으로 옮기는 diffusion decoder가 이를 보정한다.
...
*Table 19: 3D 일관성 평가. pose 추정 성공률은 실제 영상(56.4%)을 넘어선다 (NVIDIA 2025, p.37)*

Sampson error가 VideoLDM 0.841에서 7B-Text2World 0.355로, pose 추정 성공률이 4.4%에서 62.6%로 간다. novel view 합성 PSNR도 26.23에서 33.02로 올라 실제 영상 35.38에 가까워진다.

### physics alignment

PhysX와 Isaac Sim으로 자유낙하·경사면·U자 슬로프·안정 적층·불안정 적층·도미노·시소·자이로스코프 여덟 시나리오를 만들고, 카메라 4각도로 1080p 100프레임 영상 800개를 렌더해 정답으로 삼았다.

![[assets/nvidia-2025-cosmos-world-foundation-model-platform/tab20.png]]
*Table 20: physics alignment 결과. 모델을 키워도 IoU가 거의 오르지 않는다 (NVIDIA 2025, p.39)*

가장 좋은 값은 7B-Video2World에 prompt와 9프레임을 준 경우로 PSNR 21.06, 평균 IoU 0.592다. 14B로 키워도 0.598이라 사실상 제자리이고, autoregressive 계열은 0.48 근처에 몰려 있다. 화질은 나아졌지만 물리 준수는 모든 변종이 똑같이 고전한다는 게 저자들의 정리다.

### post-training 세 사례

camera control은 DL3DV-10K에 GLOMAP으로 pose를 붙여 학습했고 Plücker 좌표를 latent에 이어 붙이는 방식으로 조건을 준다.
...
CamCo와 비교하면 pose 추정 성공률 43.0% → 82.0%, 회전 오차 8.277° → 1.646°, 이동 오차 0.185 → 0.038, FID 57.49 → 14.30, FVD 433.24 → 120.49로 전 항목이 개선됐다. 학습은 DL3DV-10K, 평가는 RealEstate10K라 분포가 다른데도 그렇다.

robot manipulation은 두 갈래다. instruction 조건 영상 예측은 1X의 휴머노이드 EVE가 찍은 200시간 자체 데이터(Cosmos-1X)를 썼고, 평가자 10명이 23개 episode를 놓고 비교한 결과 전체 선호도가 78.3% 대 13.0%로 VideoLDM-Instruction을 앞섰다. action 조건 다음 프레임 예측은 Bridge 데이터셋 20,000 episode를 쓰고 action을 그리퍼 좌표계 7차원으로 받는다. FVD가 IRASim-Action 593에서 7B 모델 190으로 떨어졌다.

자율주행은 NVIDIA 내부 RDS 데이터셋을 썼다. 6개 카메라로 찍은 20초 클립 약 360만 개, 시간으로 약 2만 시간이고 차량 밀도·날씨·조도·속도·도로 유형 같은 속성 태그로 분포를 맞춰 뽑았다. 6뷰를 동시에 생성하도록 고친 결과 FID 60.84 → 32.16, FVD 884.46 → 210.23이고, 뷰 내부 시간 일관성(TSE)은 1.24 → 0.68로 실제 영상 0.69에 붙었다. 뷰 사이 일관성(CSE)은 6.48 → 2.11로 실제 영상 1.71에는 아직 못 미친다.
...
저자들은 결론에서 현재 모델이 물리 세계의 믿을 만한 시뮬레이터에 못 미친다고 못 박는다. 물체가 유지되지 않는 문제, 접촉이 많은 상황의 dynamics 부정확, instruction 따르기의 불일치를 들고, 생성 영상의 사실감이 중력·빛 상호작용·유체 같은 기본 물리 원칙 준수로 이어지지는 않는다고 적었다.
...
두 계열 비교에서는 지금까지 diffusion이 생성 품질에서 앞선다고 정리한다. camera pose·end-effector 위치·차량 trajectory 같은 다양한 조건을 붙이기 쉽고 6뷰 같은 새 출력 형식으로도 확장된다는 이유다. 다만 autoregressive 쪽에는 LLM 가중치를 물려받아 사전 지식을 상속받는 길과 causal attention용 추론 최적화 기법을 그대로 쓰는 길이 남아 있어, 두 가능성이 실현되면 판이 달라질 수 있다고 본다.

## 관련 페이지 (Related Pages)

- [[physical-ai/li-2025-a-comprehensive-survey-on-world]] — world model을 로보틱스·자율주행·범용 비디오 세 갈래로 나눈 서베이. Cosmos는 그중 general-purpose 축의 대표 사례로 놓인다
- [[physical-ai/hou-2026-world-model-for-robot-learning]] — world model을 policy로 쓰는 경로와 시뮬레이터로 쓰는 경로를 가른 서베이. Cosmos가 겨냥한 쪽은 후자다
- [[physical-ai/reuss-2026-pretrained-to-imagine-fine-tuned]] — 영상으로 pre-training한 뒤 action으로 fine-tuning하는 world-action model 흐름. Cosmos의 2단 구도와 같은 계보
- [[physical-ai/liu-2025-generative-physical-ai-in-vision]] — 생성 모델로 물리를 다루는 연구 전반. physics alignment 문제의식이 겹친다
- [[physical-ai/9bow-2026-physics-aware-generation-world-simulator]] — 물리를 이해하는 생성 모델과 world simulator를 한글로 정리한 해설
- [[physical-ai/nvidia-2025-gr00t-n1-an-open-foundation]] — 같은 NVIDIA의 휴머노이드 VLA. Cosmos가 world 쪽이라면 GR00T는 policy 쪽이고, 논문이 말하는 "자신의 디지털 트윈"과 "세상의 디지털 트윈" 짝에 해당한다
- [[physical-ai/kim-2024-openvla-an-open-source-vision-language-action-model]] — Bridge 데이터셋의 그리퍼 7차원 action 규격을 Cosmos가 그대로 따른다
