...
humanoid 로봇은 사람이 쓰도록 만들어진 작업 공간에 그대로 들어가 반복적이거나 힘든 일을 맡도록 설계된다. 그러나 예측하기 어려운 실세계 환경에서 범용으로 동작하는 humanoid를 만드는 일은 여전히 어렵다.
...
foundation model은 여러 하위 과제의 기반이 되는 대규모 범용 모델이다. 원문은 GR00T N1을 humanoid의 추론과 기술을 함께 다루는 최초의 open foundation model로 소개한다.
...
원문은 GR00T N1이 하나의 모델과 하나의 가중치로 조작 동작을 낸 humanoid를 이름으로 밝힌다. 다만 공개 자산과 달리 이 부분은 성능 주장이므로, 실제 측정이 이루어진 범위와 구분해서 읽어야 한다.
...
평가는 시뮬레이션과 실제 기기 두 종류로 나뉜다. 시뮬레이션 실험은 벤치마크 세 종을 썼고, 실제 기기 실험은 GR-1 humanoid의 탁상 조작 과제에 집중했다.
...
실제 기기 평가는 GR-1 humanoid의 탁상 조작 과제 네 묶음으로 이루어진다. 정밀한 물체 취급, 양손 협응 동작, 공간 인식이 함께 필요한 과제로 골랐다.
...
*Table 2: GR-1 humanoid 실제 기기 과제 성공률. 학습 데이터 10% 조건 (NVIDIA 2025)*
...
*Table 3: GR-1 humanoid 실제 기기 과제 성공률. 전체 데이터 조건 (NVIDIA 2025)*
...
| Fourier GR-1, 1X Neo | GR00T N1이 하나의 가중치로 동작을 낸 humanoid 두 종. 실제 기기 벤치마크는 GR-1로 측정한다 |
