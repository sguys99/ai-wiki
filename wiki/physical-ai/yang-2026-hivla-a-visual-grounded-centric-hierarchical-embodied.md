...
**flow matching.** 단순한 잡음 분포에서 목표 데이터 분포로 향하는 벡터장을 직접 학습하는 생성 모델 계열이다. diffusion model의 여러 단계 잡음 제거 대신 결정론적인 상미분방정식 적분으로 표본을 얻는다.
...
모든 블록을 통과한 뒤 action 시퀀스에 대응하는 hidden state는 MLP 기반 디코더를 거쳐 로봇의 원래 action 공간으로 사상된다. 이 디코더도 시각 임베딩으로 변조된다.
