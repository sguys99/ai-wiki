...
마지막 문제는 상황 분기다. 경로 계산 실패, 경로 추종 실패, 목표 갱신처럼 처리해야 할 경우가 늘어나면 Finite State Machine(FSM)은 상태 수십 개와 전이 수백 개로 늘어난다. Nav2가 ROS 1의 move_base와 가장 크게 달라진 부분인 Behavior Tree가 이 문제에 대한 답이다.
