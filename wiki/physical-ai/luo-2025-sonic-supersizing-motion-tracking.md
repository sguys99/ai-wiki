...
GMR과 PyRoki로 Unitree G1에 retarget한 뒤 계단 오르기나 착석처럼 대상 로봇이 실행할 수 없는 동작을 걸러 611시간을 남겼다. retargeting은 사람 동작 데이터를 로봇 형상에 맞게 변환하는 과정을 말한다. 남은 데이터는 50Hz 기준 1억 프레임 이상, 클립 수로는 317,189개이고 33개 main category 아래 sub-category로 나뉜다. 기본과 고급 locomotion, 제스처, 댄스, 전투, 물체 조작, tool use, 부상 보행, 스타일 변형, 역할극이 포함되며 모든 모션은 좌우 미러링으로 쌍을 이룬다.
