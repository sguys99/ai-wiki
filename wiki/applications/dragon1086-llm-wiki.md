...
이 저장소를 특징짓는 것은 명령 표면이 좁다는 점이다. `ingest`가 자료를 페이지로 바꾸고, `query`가 wiki를 탐색해 답을 만들고, `lint`가 링크와 인덱스의 건강을 점검한다. 여기에 `watch`가 폴더 감시를 상시화하고 `discord`가 같은 기능을 채팅 명령으로 노출한다. 세 개의 핵심 동사와 세 개의 보조 명령으로 지식 베이스 운영 전체를 덮는 구성이다.
...
| index 불일치 | 실제 파일 목록과 인덱스 내용이 어긋난 상태 | 파일은 있는데 인덱스에 없거나, 인덱스에는 있는데 파일이 없다 |
...
| `./wiki lint --fix` 실행 | 2주에 한 번 | 링크와 인덱스의 어긋남을 정리한다 |
