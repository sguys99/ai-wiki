...
이 파일은 `wiki/` 안의 모든 페이지를 카탈로그화한 인덱스다. 자료가 추가될 때마다 해당 카테고리 섹션에 한 줄씩 추가한다.
...
- [[database/guo-2025-lightrag-simple-and-fast|LightRAG]]: entity와 relation을 key-value로 인덱싱하고 질의에서 구체와 추상 키워드를 함께 뽑는 dual-level retrieval로, community 처리 없이 검색 토큰을 61만에서 100 미만으로 줄였다 (2025, paper)
...
- [[database/geeksforgeeks-2026-vectorless-rag-pageindex|Vectorless RAG: PageIndex 튜토리얼]]: PageIndex 클라우드 API와 LangChain과 Gemini로 PDF를 인덱싱하고 질의하는 코드 10단계 입문 튜토리얼. 벤치마크 수치는 없다 (2026, article)
...
- [[database/gutierrez-2025-from-rag-to-memory-non|HippoRAG 2: From RAG to Memory]]: KG에 passage 노드를 더하고 질의를 triple에 이어, 사실 기억과 sense-making과 associativity 세 메모리 과제를 동시에 개선한 논문 (2025, paper)
...
- [[physical-ai/jo-2026-gemini-robotics-1-5-vla-primer|03-16. Gemini Robotics 1.5 (모두의 로보틱스)]]: 1.5의 한국어 입문 해설. orchestrator, Thinking VLA, Motion Transfer를 예시로 세운 뒤 두 모델의 분업을 짚는다 (2026, article)
...
- [[physical-ai/google-deepmind-2025-gemini-robotics-15-pushing-the-frontier|Gemini Robotics 1.5 (Google DeepMind)]]: 세 로봇을 checkpoint 하나로 움직이는 VLA와 ER VLM을 묶은 agentic system 기술 보고서 (2025, paper)
- [[physical-ai/deepmind-2025-gemini-robotics-15-brings-ai-agents|Gemini Robotics 1.5 발표문 (DeepMind)]]: 같은 날 나온 공식 발표문. agentic framework와 세 능력을 분리수거 예시로 설명하고 ER 1.5 API 공개를 알린다 (2025, article)
...
- [[physical-ai/huang-2026-mobilevla-r1-2-0-rl-enhanced-reasoning-for|MobileVLA-R1 2.0 (Peking University)]]: chain-of-thought 내부 표현을 decoder로 읽어 속도 명령을 내는 이동 로봇 VLA. Unitree G1에 그대로 옮겼다 (2026, paper)
...
- [[physical-ai/li-2026-tango-humanoid-navigation-in-cluttered|TANGO (UC Berkeley)]]: 지시문과 RGB만으로 29-DoF whole-body action을 직접 예측하는 VLN 프레임워크. 시뮬레이터에서만 학습해 Unitree G1에 zero-shot 배포했다 (2026, paper)
...
- [[physical-ai/natnew-awesome-physical-ai|Awesome Physical AI (natnew, repo)]]: Physical AI 도구, 데이터, 논문, 규제 문서 353개를 14개 카테고리로 모은 큐레이션 인덱스. 개별 자료로 가는 지도로 쓰며 라이선스는 MIT다 (2026, repo)
...
- [[physical-ai/acensia-long-horizon-papers|long-horizon-papers (acensia, repo)]]: long-horizon 논문 52편을 LLM agent, VLM planning, VLA manipulation으로 나눈 큐레이션 저장소. VLA는 해결 방식별로 다섯 폴더로 나눈다 (2026, repo)
...
- [[physical-ai/strands-labs-robots|Strands Robots (strands-labs, repo)]]: Strands Agent가 자연어로 로봇을 움직이게 하는 Apache-2.0 라이브러리. teleoperation 수집부터 policy 학습, 실제 기기 실행, mesh, ROS 2 연결까지 담았다 (2026, repo)
- [[physical-ai/enactic-openarm|OpenArm (Enactic, repo)]]: 7자유도 오픈소스 humanoid 팔에 평가 환경 Cell과 모터 없는 leader 팔 KER를 더한 Apache-2.0 프로젝트. bilateral teleoperation 수집부터 ACT 학습과 Dora 추론까지 문서화했다 (2026, repo)
- [[physical-ai/ma-2026-current-as-touch-proprioceptive-contact|Current as Touch (UNC)]]: 촉각 센서 없이 모터 전류로 접촉을 감지하고 PD 제어기용 compliance reference position을 예측하는 dexterous hand 프레임워크 (2026, paper)
- [[physical-ai/ma-2026-current-as-touch-project-page|Current as Touch 프로젝트 페이지]]: Current as Touch 논문의 공식 프로젝트 페이지. 과제 네 개의 시연 영상과 전류 유무 스틸 컷, 대표 수치 카드를 배열한 짧은 소개 페이지다 (2026, article)
...
- [[physical-ai/iovino-2024-comparison-between-behavior-trees-and|Behavior Tree와 FSM 비교 (ABB, ETH, KTH)]]: 두 policy 표현을 mobile manipulation에서 모듈성, 반응성, 가독성으로 비교. 동작은 같지만 큰 과제에서 BT 편집이 적다 (2024, paper)
- [[physical-ai/suzuki-2026-from-dialogue-to-execution-mixture-of-agents|MoA 대리 응답과 BT 실행 (Waseda, NII)]]: 확인 질문을 세 expert agent가 나눠 답하고 나머지만 사람이 답하는 대화형 계획. BT action node마다 policy를 묶는다 (2026, paper)
- [[physical-ai/choi-2026-reactree-hierarchical-llm-agent-trees|ReAcTree (ETRI, UST)]]: subgoal별 LLM agent를 BT식 control flow로 엮어 실행 중에 키우는 task planning. WAH-NL goal 성공률이 31%에서 61%로 올랐다 (2026, paper)
- [[physical-ai/ao-2024-llm-as-bt-planner-leveraging-llms-for-behavior|LLM-as-BT-Planner (TUM MIRMI, MBZUAI)]]: LLM이 로봇 조립 계획을 실행 가능한 BT로 직접 생성한다. 네 방식 중 human-in-the-loop이 17개 중 16개 성공 (2024, paper)
...
- [[physical-ai/peng-2026-cortex-a-bidirectionally-aligned-embodied|Cortex (Shanghai AI Lab)]]: VLM 계획을 32개 skill primitive로 묶어 하위 VLA가 실행하는 dual-system VLA. 14단계 화학 실험 zero-shot 65% 성공 (2026, paper)
...
- [[physical-ai/hu-2026-cst-wm-a-causally-structured-world|CST-WM (NYU Abu Dhabi)]]: embodied visual tracking용 world model. action이 target evidence를 직접 갱신하는 경로를 끊고 MPC로 추종과 재획득을 함께 처리한다 (2026, paper)
...
- [[agents/garrytan-gstack|gstack (repo)]]: Garry Tan의 Claude Code skill pack. 역할별 슬래시 커맨드로 개발을 스프린트 사이클로 구조화한다 (2026, repo)
- [[agents/9bow-2026-gstack-claude-code-virtual-team|gstack 명령어 카탈로그 (9bow)]]: PyTorch KR이 gstack의 슬래시 커맨드를 스프린트 사이클에 맞춰 5개 그룹으로 정리한 한국어 카탈로그. 설치 3종과 텔레메트리 범위까지 담았다 (2026, article)
- [[agents/gpters-2026-yc-ai-agent-guide-gstack|Y Combinator의 AI 에이전트 사용법 완벽 정리 (GPTers)]]: gstack 9개 슬래시 커맨드의 사용법 가이드. 브라우저 QA 데몬의 콜드 스타트 3~5초, 이후 호출 100~200ms 수치를 담았다 (2026, article)
...
- [[agents/headroomlabs-ai-headroom|Headroom (repo)]]: 에이전트가 읽는 tool 출력과 로그, RAG chunk를 LLM 도달 전에 압축해 토큰을 60~95% 줄인다는 context compression layer. library, proxy, wrap 세 형태로 도입한다 (2026, repo)
...
- [[applications/safishamsi-graphify|safishamsi/graphify (repo)]]: 임의의 폴더를 읽어 knowledge graph로 만드는 Claude Code 스킬. 원본 직접 읽기 대비 질의당 토큰 71.5배 절감을 보고하고, 엣지마다 발견과 추측을 라벨로 구분한다 (2026, repo)
...
- [[applications/shamsi-2026-graphify-knowledge-graphs-for-ai|Graphify 공식 제품 페이지]]: 코드와 문서, 논문, 다이어그램을 임베딩 없이 하나의 knowledge graph로 묶는 오픈소스 스킬. 제품이 스스로 제시한 71.5배 토큰 감축 수치와 그 근거를 정리했다 (2026, article)
